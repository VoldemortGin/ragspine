...
class PageInput:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            isfinite(value) and value > 0 for value in (self.width, self.height)
        ):
...
class ProcessingManifest:
    ...
    def __post_init__(self) -> None:
        if (
            tuple(page.page_index for page in self.pages)
            != self.scope.selected_page_indices
        ):
            raise ValueError(
                "Processing manifest must cover every selected page exactly"
            )
