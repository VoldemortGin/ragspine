...
class RetrievalMember:
    ...
    def __post_init__(self) -> None:
        if (
            not self.object_id
            or not self.embedding_fingerprint
            or self.embedding_dimensions <= 0
        ):
            raise ValueError(
                "Retrieval members require explicit source and embedding identities"
            )
...
class RetrievalPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            member.page_index not in self.scope.selected_page_indices
...
def retrieval_dependencies(plan: RetrievalPlan) -> tuple[AssetRef, ...]:
    ...
    return tuple(
        sorted(refs, key=lambda ref: (ref.sha256, ref.media_type, ref.byte_length))
    )
...
def resolve_member(plan: RetrievalPlan, hit: PinnedRetrievalHit) -> RetrievalMember:
    ...
    member = next(
        (item for item in plan.members if item.member_id == hit.member_id), None
    )
    if member is None:
