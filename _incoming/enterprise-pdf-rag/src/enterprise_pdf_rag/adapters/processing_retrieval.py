...
class ProcessingRetrieval:
    ...
    def _embedding(
        self, description: AssetRef, text: str
    ) -> tuple[AssetRef, RetrievalEmbedding]:
        fingerprint = sha256(
            repr(
                ("description-embedding-v1", description, self.embedder.fingerprint)
            ).encode()
        ).hexdigest()
        ...
        if cached is not None:
            ...
            if (
                ...
                raise ValueError(
                    "Cached embedding belongs to a different description or model"
                )
    ...
    def search(
        ...
        if not plan.members:
            return ()
        if any(
            member.embedding_fingerprint != self.embedder.fingerprint
        ...
        for entry in index.entries:
            if len(query_vector) != len(entry.vector):
                raise ValueError(
                    "Query embedding dimensions differ from the pinned index"
                )
            divisor = sqrt(
            ...
            score = (
                sum(
                    left * right
                    for left, right in zip(query_vector, entry.vector, strict=True)
                )
                / divisor
                if divisor
                else 0.0
...
def resolve_processing_context(
    ...
    if member.kind is ObjectKind.CHART:
        ...
        return RetrievalContext(
            plan.snapshot_id, member, chart, chart_description, chart_receipt
        )
    ir, description, receipt = validate_literal_member(
        sources, outputs.assets, plan.scope, member
    )
