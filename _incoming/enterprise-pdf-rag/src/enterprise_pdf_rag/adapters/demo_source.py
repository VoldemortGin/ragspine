...
def make_demo_figure() -> tuple[bytes, SvgArtifact]:
    ...
    artifact = PdfspineFigureParser().extract(
        pdf, page_index=0, bbox=(20.0, 15.0, 250.0, 225.0)
    )
    ...
    if (
        labels != {"Revenue", "USDm", "2024", "10", "2025", "15"}
        or shapes != expected_bounds
    ):
        raise ValueError("Demo source observations differ from the authored fixture")
    if (
        len(root.findall(namespace + "rect")) != 2
        or len(root.findall(namespace + "text")) != 6
    ):
