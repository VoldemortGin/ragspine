...
class ModelPagePartitioner:
    ...
    def partition(self, page: PageInput) -> PagePartition:
        ...
        observations = [
            {"id": alias, "text": span.text, "bbox": span.bbox}
            for alias, span in aliases.items()
        ]
        prompt = (
        ...
        try:
            ...
            unassigned = tuple(
                aliases[alias].span_id for alias in output.unassigned_span_ids
            )
        except KeyError:
            raise ValueError(
        ...
        objects = tuple(
            replace(
                ...
                child_object_ids=tuple(
                    ...
                    if child.parent_id == item.object_id
                ),
            )
            for item in objects
        )
