...
class PreparedFigure:
    ...
    def context_artifact_id(self) -> str | None:
        return (
            content_id("page-context-v1", self.page_context)
    ...
    *,
    ...
    for span_id in context_span_ids:
        ...
        if not _inside(span.bbox, (0.0, 0.0, page.width, page.height)) or (
            ...
            raise ValueError(
                "Page context must be on the source page and outside the crop"
            )
        context.append(
    ...
    for span in page.text.spans:
        ...
        for start, end in _fragments(span):
            element_id = (
                "obs-"
                + sha256(f"{span.span_id}:{start}:{end}".encode()).hexdigest()[:16]
            )
    ...
    paint_spans = tuple(
        span
        for span in page.text.spans
        if _inside(span.bbox, bbox) or span.span_id in excluded
    )
    return PreparedFigure(
        svg, crop, rendered, view, tuple(excluded), tuple(context), paint_spans
    )
