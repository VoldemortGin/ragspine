...
def _prepare(
    *, page: PageInput, item: LayoutObject, native_svg: bytes
) -> _PreparedVisual:
    if (
    ...
    if any(
        not _inside(observed[span_id].bbox, item.bbox)
    ...
    for span_id in item.source_span_ids:
        ...
        start, end = offsets
        element_id = (
            "obs-" + sha256(f"{span.span_id}:{start}:{end}".encode()).hexdigest()[:16]
        )
    ...
    structured = (
        crop[:-6] + "<metadata>" + "".join(metadata) + "</metadata></svg>"
        if metadata
        else crop
    )
    svg = SvgArtifact(
    ...
    all_full = tuple(
        span.span_id for span in page.text.spans if _inside(span.bbox, item.bbox)
    )
    partial = tuple(
        span.span_id
    ...
    unowned = tuple(
        span_id for span_id in all_full if span_id not in item.source_span_ids
    )
    view = {
...
def _elements(
    prepared: _PreparedVisual, evidence: VisualEvidenceDTO
) -> tuple[SvgElement, ...]:
    ...
    if any(
        element.source_span_id not in prepared.full_span_ids for element in elements
    ):
...
class VisualSemanticAdapter:
    ...
    def infer(
        self, *, page: PageInput, item: LayoutObject, native_svg: bytes
    ) -> VisualInference:
        if item.kind not in (
            ObjectKind.IMAGE,
            ObjectKind.DIAGRAM,
        ...
        ir, ir_raw, ir_diagnostic, confidence = self._infer_ir(
        ...
        *,
        ...
        if isinstance(dto, DiagramObservationsDTO):
            ...
            for node in dto.nodes:
                ...
                if not _label_matches(node.label, elements) or not _inside(
                    node.bbox, item.bbox
                ):
