...
def _schema_arrays(value: object) -> object:
    ...
    if prefix is not None:
        if (
            ...
            or not prefix
            or any(item != prefix[0] for item in prefix)
        ):
...
def _response_schema(
    response_model: type[BaseModel], bound_svg_digest: str | None
) -> object:
    ...
    def _complete[T: BaseModel](
        ...
        *,
        ...
        try:
            ...
            raise JsonCompletionError(
                code, fingerprint, diagnostics=diagnostic
            ) from None
        diagnostic = RequestDiagnostics(
            endpoint_path="/v1/chat/completions",
            request_bytes=len(payload),
        ...
        try:
            ...
            raise JsonCompletionError(
                error.code, fingerprint, diagnostics=diagnostic
            ) from None
