...
def _evidence(*element_ids: str) -> Evidence:
    return Evidence(
        ...
        Confidence(
            None, "offline-demo: explicit source labels; no calibrated model score"
        ),
    )
    ...
    def extract(self, svg: SvgArtifact) -> ChartIR:
        ...
        points = tuple(
            ChartPoint(
                ...
                unit,
                NumericObservation(
                    Decimal(value), ValueKind.EXPLICIT, _evidence(labels[value])
                ),
            )
    ...
    def generate(self, svg: SvgArtifact) -> TextDescription:
        ...
        claims = tuple(
            DescriptionClaim(
                ...
                _evidence(
                    labels["Revenue"], labels["USDm"], labels[year], labels[value]
                ),
                series="Revenue",
                category=year,
                unit="USDm",
