...
class DescriptionNormalizationReceipt(_Strict):
    ...
    producer: Literal["duplicate-evidence-normalization-v1"] = (
    ...
    def artifact_id(self) -> str:
        return (
            "description-normalization-v1:"
            + sha256(self.model_dump_json().encode()).hexdigest()
        )
...
def normalize_description_evidence(
    *,
    ...
    if original_mapping.description != previous:
        raise ValueError(
            "Normalization lineage differs from the original typed description"
        )
    changes = tuple(
                ...
                claim.model_copy(
                    update={
                        "evidence": claim.evidence.model_copy(
                            update={
                                "element_ids": tuple(
                                    dict.fromkeys(claim.evidence.element_ids)
                                ),
                            }
                        )
                    }
