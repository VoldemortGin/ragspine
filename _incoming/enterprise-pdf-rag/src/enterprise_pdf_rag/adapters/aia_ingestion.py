...
def read_text_sidecar(
    ...
    if (
        ...
        or sidecar.source_sha256 != snapshot.manifest.source.sha256
    ):
        raise ValueError("Text sidecar is not bound to this source page")
    return sidecar


def read_region_sidecar(
    store: LocalDocumentStore, snapshot: DocumentSnapshot
) -> TextSidecar:
    ...
    sidecar = TypeAdapter(TextSidecar).validate_json(
        store.get(region.text), strict=True
    )
    if (
        sidecar.page_index != region.page_index
        or sidecar.source_sha256 != manifest.source.sha256
    ):
        raise ValueError("Source region text is not bound to this source page")
    observed = {
        span.span_id: span
        for span in read_text_sidecar(store, snapshot, region.page_index).spans
    }
...
def source_text_json(store: LocalDocumentStore, snapshot: DocumentSnapshot) -> str:
    sidecars = [
        asdict(read_text_sidecar(store, snapshot, i))
        for i in range(len(snapshot.manifest.pages))
    ]
    return (
        json.dumps(
            {
...
def ingest_aia(
    *,
    ...
    try:
        ...
        attempt.update(
            state="failed", diagnostic=str(error), completed_at_unix=time.time()
        )
        attempt_path.write_text(json.dumps(attempt, indent=2) + "\n")
        raise
    attempt.update(
        state="complete", manifest_id=manifest_id, completed_at_unix=time.time()
    )
    attempt_path.write_text(json.dumps(attempt, indent=2) + "\n")
    return snapshot
