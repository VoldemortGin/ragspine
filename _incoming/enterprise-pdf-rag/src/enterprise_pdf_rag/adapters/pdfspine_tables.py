...
class PdfspineTableAdapter:
    ...
    def extract(
        self, pdf: bytes, *, page: PageInput, item: LayoutObject
    ) -> TableExtractionResult:
        ...
        try:
            ...
            try:
                detected = tuple(
                    source_page.find_tables(strategy="lines", clip=item.bbox)
                )
                matches = tuple(
                    table
                    for table in detected
                    if _contains(item.bbox, _bounds(table.bbox))
                )
    ...
    def _map_table(
        ...
        for native in table.origin_cells:
            ...
            source_span_ids = tuple(
                span.span_id
                for span in spans_in_table
                if _center_inside(span.bbox, bbox)
            )
