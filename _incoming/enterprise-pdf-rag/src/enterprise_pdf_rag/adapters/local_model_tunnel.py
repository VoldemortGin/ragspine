...
def tunnel_status(
    config: LocalModelTunnelConfig, state_directory: Path
) -> TunnelStatus:
...
def start_tunnel(
    ...
    if _port_ready(config.embedding_local_port) or _port_ready(
        config.rerank_local_port
    ):
    ...
    while process.poll() is None and time.monotonic() < deadline:
        if _port_ready(config.embedding_local_port) and _port_ready(
            config.rerank_local_port
        ):
