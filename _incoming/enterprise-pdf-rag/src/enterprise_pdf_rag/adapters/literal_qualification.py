...
def validate_literal_member(
    ...
    description = TypeAdapter(ObjectDescription).validate_json(
        assets.get(member.description)
    )
    if description.producer != "exact-source-transcription-v1":
        raise ValueError(
            "Literal qualification requires the exact transcription producer"
        )
    ...
    if (
        ...
        raise ValueError(
            "Literal qualification does not bind the exact object dependencies"
        )
    ...
    if (anchor.coordinate_frame, anchor.rotation, anchor.transform) != (
        ...
        raise ValueError(
            "Literal source anchor has an unsupported coordinate transform"
        )
    ...
    for span_id in receipt.source_span_ids:
        ...
        if not (
            ...
            raise ValueError(
                "Literal source occurrence is outside its qualified anchor"
            )
    ...
    if assets.get(member.source_svg) != expected_crop:
        raise ValueError(
            "Literal SVG crop does not derive from the pinned source page and anchor"
        )
    ...
    if (
        ...
        or ir.source != anchor
        or tuple(fragment.source_span_id for fragment in ir.fragments)
