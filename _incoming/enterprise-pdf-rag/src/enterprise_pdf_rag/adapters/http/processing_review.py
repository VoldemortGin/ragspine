...
def processing_status(
    processing_id: str, manifest: ProcessingManifest
) -> ProcessingStatusResponse:
    ...
    return ProcessingStatusResponse(
        ...
        ir_artifacts=sum(
            stage.stage == "ir" and stage.artifact is not None for stage in stages
        ),
        description_artifacts=sum(
            stage.stage == "description" and stage.artifact is not None
            for stage in stages
        ),
    ...
    def context(body: ProcessingContextRequest) -> ProcessingContextResponse:
        ...
        try:
            result = resolve_processing_context(
                ...
                manifest.retrieval,
                PinnedRetrievalHit(
                    body.hit.snapshot_id, body.hit.member_id, body.hit.score
                ),
            )
        except (ValueError, OSError):
            raise HTTPException(
