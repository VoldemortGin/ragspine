...
def completion_events(result: CompletionResponse) -> Iterator[str]:
    ...
    deltas.extend(
        CompletionDelta(content=text[start : start + 128])
...
def create_demo_router(runtime: Runtime) -> APIRouter:
    ...
    def completion(body: ChatRequest) -> CompletionResponse | StreamingResponse:
        if body.model != DEMO_MODEL:
            raise HTTPException(
                404, "Only the explicitly named offline-demo model is available"
            )
        ...
        hit = next(
            (
                ...
                if candidate.bundle_id == pinned.bundle_id
            ),
            None,
        )
        if hit is None:
            raise HTTPException(
                409, "Pinned demo evidence is unavailable; no summary fallback"
            )
        ...
        result = CompletionResponse(
            ...
            choices=(
                CompletionChoice(
                    message=ChatMessage(role="assistant", content=_render(view))
                ),
            ),
        )
        if body.stream:
