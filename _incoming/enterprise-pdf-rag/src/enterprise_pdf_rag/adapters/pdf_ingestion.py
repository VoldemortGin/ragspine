...
def _source(
    ...
    ref = AssetRef(
        manifest_id, "application/json", len(sources.read_content(manifest_id))
    )
    cache.cache(
        StageOutcome("source", fingerprint, StageState.SUCCEEDED, producer, ref)
    )
    ...
    *,
    ...
    parent = (
        (
            ...
            else get_settings().data_dir / "ingestion"
        )
        .expanduser()
        .resolve()
    )
    ...
    source, selected, cached = _source(
        sources, pdf=data, filename=pdf.name, pages=pages
    )
    ...
    processing_id, manifest = pipeline.run(
        source.manifest_id, selected_page_indices=selected
    )
