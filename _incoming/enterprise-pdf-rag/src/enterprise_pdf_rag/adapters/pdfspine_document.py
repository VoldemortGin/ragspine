...
def _spans(
    ...
    for block_index, block in enumerate(text.blocks):
        ...
        for line_index, line in enumerate(_RECORDS.validate_python(block.get("lines"))):
            for span_index, raw in enumerate(
                _RECORDS.validate_python(line.get("spans"))
            ):
...
def _extract_page(
    page: pdfspine.Page, *, source_digest: str, page_index: int
) -> PageExtraction:
...
def _checked_page(
    ...
    try:
        ...
        raise ValueError(
            f"Page index {page_index} extraction failed: {error}"
        ) from error
...
class PdfspineDocumentAdapter:
    ...
    def extract_document(self, pdf: bytes) -> DocumentExtraction:
        ...
        try:
            pages = tuple(
                _checked_page(
                    document, source_digest=source_digest, page_index=page_index
                )
    ...
    def extract_region(
        ...
        cropped = crop_native_svg(
            page.native_svg, width=page.width, height=page.height, bbox=bbox
        )
