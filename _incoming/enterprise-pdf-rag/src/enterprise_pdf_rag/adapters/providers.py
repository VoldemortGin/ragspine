...
def _required(environment: Mapping[str, str], name: str) -> str:
    ...
    if not value or "\n" in value or "\r" in value:
        raise ProviderConfigurationError(
            f"Missing or invalid environment setting: {name}"
        )
    return value
...
def load_llm_config(environment: Mapping[str, str] | None = None) -> LLMConfig:
    ...
    base = _base_url(
        _required(env, "OPENAI_BASE_URL"), name="OPENAI_BASE_URL", https_only=True
    )
...
class SmokeSender(Protocol):
    def __call__(
        self, url: str, *, api_key: str, payload: bytes, timeout: float
    ) -> bytes: ...
...
class OpenAICompatibleSmoke:
    ...
    def run(self) -> SmokeResult:
        ...
        try:
            ...
            if len(choices) != 1:
                raise ProviderRequestError(
                    "Provider returned an unexpected choice count"
                )
            ...
            raise ProviderRequestError(
                "Provider returned an invalid completion schema"
            ) from None
        return SmokeResult(
            ok=True,
            model=self._config.model,
