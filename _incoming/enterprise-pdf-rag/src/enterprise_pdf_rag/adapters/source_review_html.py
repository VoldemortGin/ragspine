...
def _validate_text(
    snapshot: DocumentSnapshot, page_index: int, text: TextSidecar
) -> None:
    if (
        ...
        raise ValueError(
            "Source page/text identity does not match the selected snapshot"
        )
...
def render_index(
    *, snapshot: DocumentSnapshot, region_svg: str, region_text: TextSidecar
) -> str:
    ...
    if (
        manifest.source.sha256 == AIA_SPEC.sha256
        and region.page_index == AIA_SPEC.focus_page_index
    ):
        warnings += (
...
def render_page(
    *, snapshot: DocumentSnapshot, page_index: int, svg: str, text: TextSidecar
) -> str:
    ...
    warnings = "".join(
        ...
        for message in snapshot.manifest.pages[page_index].warnings
    )
    body = (
