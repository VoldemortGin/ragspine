...
class _Writer:
    ...
    def save(
        ...
        outcome = StageOutcome(
            stage, fingerprint, StageState.SUCCEEDED, self.producer, ref
        )
        self.outputs.cache(outcome)
        return outcome

    def diagnostic(
        self, stage: str, reason: str, *, failed: bool = False
    ) -> StageOutcome:
        fingerprint = sha256(
            repr(
                (
...
class SemanticObjectAdapter:
        ...
        *,
        ...
        if (
            ...
            or any(
                ...
                for value in description_corrections
            )
        ):
            raise ValueError(
        ...
        if len(chart_corrections) > 1 or any(
            ...
            raise ValueError(
                "At most one explicit original chart request fingerprint is allowed"
            )
    ...
    def process(self, page: PageInput, item: LayoutObject) -> ObjectProcessingRecord:
        if item.kind in (ObjectKind.TEXT, ObjectKind.LIST, ObjectKind.GROUP):
            return ProcessingObjectAdapter(self.sources, self.outputs).process(
                page, item
            )
        writer = _Writer(
            self.outputs,
        ...
        if self.description_corrections:
            writer = _Writer(
                ...
                + ":corrections:"
                + sha256(
                    repr(tuple(sorted(self.description_corrections))).encode()
                ).hexdigest(),
            )
        ...
        sidecar = TextSidecar(
            ...
            page.page_index,
            tuple(
                span for span in page.text.spans if span.span_id in item.source_span_ids
            ),
        )
        stages = [
        ...
        for name, value, raw, diagnostic in (
            ...
            stages.append(
                ...
                if value is None
                    ...
                    name, TypeAdapter[object](type(value)).dump_json(value)
                )
            )
        stages.append(
            writer.diagnostic(
    ...
    def _chart(
        ...
        view = writer.save(
            "model_view",
            TypeAdapter[object](type(prepared.model_view)).dump_json(
                prepared.model_view
            ),
        )
        stages.extend(
            (svg, view, writer.save("model_render", prepared.rendered.png, "image/png"))
        )
        ...
        if self.qualification_policy == "none":
            ...
            or chart_stage is None
            or description_stage is None
        ):
            stages.append(
                writer.diagnostic(
                    "qualification",
        ...
        return ObjectProcessingRecord(
            item.object_id, item.kind, tuple(stages), qualified_count
        )
    ...
    def _unavailable(
        ...
        stages.extend(
            (
                writer.save(
                    "svg", writer.outputs.assets.get(_ref(crop)), "image/svg+xml"
                ),
        ...
        return ObjectProcessingRecord(
            writer.item.object_id, writer.item.kind, tuple(stages)
        )
    ...
    def _qualify(
        ...
        ir = writer.save(
            "qualified_ir", TypeAdapter(ChartIR).dump_json(projected_chart)
        )
        desc = writer.save(
