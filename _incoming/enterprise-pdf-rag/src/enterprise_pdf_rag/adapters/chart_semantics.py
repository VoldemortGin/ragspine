...
def _evidence(svg: SvgArtifact, dto: EvidenceDTO, field_path: str) -> Evidence:
    ...
    ordinal = (
        dto.confidence
        if dto.confidence in {"high", "medium", "low", "unknown"}
        else None
    )
    score = None
    if ordinal is None:
...
def _numeric(
    ...
    if any(
        re.search(r"[<>≤≥]\s*[+-]?\d|\(\s*[+-]?\d", literal) for literal in literals
    ):
        diagnostics.append(
...
def _prompt(prepared: PreparedFigure) -> str:
    ...
    if prepared.page_context:
        payload["page_context"] = [
            {"id": context.observation_id, **asdict(context)}
...
def _check_view(svg: SvgArtifact, prepared: PreparedFigure) -> None:
    ...
    if any(
        ...
        or context.source_span_id
        in {element.source_span_id for element in svg.elements}
        for context in prepared.page_context
    ):
    ...
    *,
    ...
    if prior.request_fingerprint != correction_of:
        raise JsonCompletionError(
            "correction_reference_mismatch", prior.request_fingerprint
        )
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            and re.fullmatch(r"[0-9a-f]{64}", correction_of) is None
        ):
            raise ValueError(
                "Correction requires an immutable original request fingerprint"
            )
        self._client = client
