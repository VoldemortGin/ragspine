...
def replace_index_member(
    ...
    revised = replace(
        plan,
        members=tuple(
            member if item.member_id == old_member_id else item for item in plan.members
        ),
        qualification_policy=PROMOTION_POLICY,
    )
    vectors = {
        member.member_id if key == old_member_id else key: value
...
def _promoted_record(
    ...
    if not {"qualified_ir", "qualification"}.issubset(
...
def create_numeric_draft(
    ...
    *,
    ...
    promoted = promote_numeric_label_member(
        sources, outputs.assets, manifest.scope, old
    )
    ...
    if not isinstance(receipt, NumericLabelPublicationReceipt):
        raise ValueError(
            "Promotion requires the source-paint numeric publication policy"
        )
    ...
    pages = tuple(
        replace(
            ...
            objects=tuple(
                _promoted_record(
                    record, promoted, receipt.source_paint_proof, claim_count
                )
                if (page.page_index, record.object_id)
                == (promoted.page_index, promoted.object_id)
                else record
