...
class ReviewedPaintExclusion:
    ...
    def review_id(self) -> str:
        return (
            f"reviewed-source-paint:{self.rule_version}:"
            + sha256(repr(self).encode()).hexdigest()
        )
...
def _review_exclusion(
    ...
    if (
        ...
        or min(bbox[2], hull[2]) - max(bbox[0], hull[0])
        > review.maximum_visible_width + 1e-8
    ):
...
def _path_reference(
    node: ElementTree.Element, matrix: Matrix, clips: tuple[Bounds, ...]
) -> str:
    return sha256(
...
def distance(point: Point, first: Point, last: Point) -> float:
    ...
    t = (
        ...
        if length == 0
            ...
            0.0,
            min(
                1.0, ((point[0] - first[0]) * dx + (point[1] - first[1]) * dy) / length
            ),
        )
    )
...
def contains(points: tuple[Point, ...], point: Point, *, margin: float = 0.0) -> bool:
    ...
    for first, last in zip(points, points[1:] + points[:1], strict=True):
        if distance(point, first, last) <= margin:
            return False
        if (first[1] > point[1]) != (last[1] > point[1]) and point[0] < (
            last[0] - first[0]
        ) * (point[1] - first[1]) / (last[1] - first[1]) + first[0]:
...
def _curve(
    first: Point, c1: Point, c2: Point, last: Point, depth: int = 0
) -> tuple[Point, ...]:
    ...
    middle = _mid(d, e)
    return _curve(first, a, d, middle, depth + 1) + _curve(
        middle, e, c, last, depth + 1
    )
...
def _matrix(source: str) -> Matrix:
    ...
    values = tuple(
        float(part) for part in re.split(r"[,\s]+", matched.group(1).strip())
    )
...
def _path_controls(source: str) -> tuple[Point, ...]:
    ...
    while index < len(tokens):
        ...
        if command in {"H", "V"}:
            current = (
                (values[0], current[1]) if command == "H" else (current[0], values[0])
            )
...
def _annulus(sectors: tuple[NativeSector, ...]) -> None:
    ...
    radial = tuple(
        hypot(x - center[0], y - center[1])
        for sector in sectors
        for x, y in sector.points
    )
    inner = min(radial)
    tolerance = 0.15
    if (
    ...
    for sector in sectors:
        ...
        rings = tuple(
            0 if abs(radius - inner) < abs(radius - outer) else 1 for radius in radii
        )
        if any(
    ...
    *,
    ...
    def walk(
        ...
        unresolved_paint = (
            ...
            or any(
                node.get(name)
                for name in ("mask", "filter", "style", "display", "visibility")
            )
            or any(
        ...
        if tag == "path" and (fill != "none" or stroke != "none"):
            ...
            hull = _bounds(
                tuple(
                    _transform(matrix, point)
                    for point in _path_controls(node.get("d", ""))
                )
            )
            paint_bounds = hull
            if stroke != "none":
            ...
            if fill.lower() in {"#000000", "#ffffff"}:
                ...
                if len(spans) != 1 or any(
                    not _inside(hull, clip_bounds) for clip_bounds in clips
                ):
