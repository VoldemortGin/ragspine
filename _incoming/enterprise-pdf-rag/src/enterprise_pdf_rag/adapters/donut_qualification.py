...
class DonutQualification:
    ...
    def _source(
        ...
        inner = min(
            hypot(p[0] - center[0], p[1] - center[1])
        ...
        for literal in literals:
            ...
            if len(categories) != 1:
                raise _fail("category_to_numeric_label_relation_ambiguous")
            fragments = tuple(
                e for e in svg.elements if e.source_span_id == literal.source_span_id
            )
            ...
            values.append(
                _SourcePoint(categories[0], literal, number, unit, Decimal(number.text))
            )
    ...
    def qualify_pair(
        ...
        for point in chart.points:
            ...
            if (
                ...
                or point.value.value.as_tuple().exponent
                != source.value.as_tuple().exponent
            ):
            ...
            if any(
                ...
                e.text in {source.literal.text, source.number.text} for e in references
            ):
        ...
        for claim in description.claims:
            ...
            claims.append(
                replace(
                    claim, text=projected, evidence=_verified(claim.evidence, *expected)
                )
            )
        ...
        for point in qualified_points:
            fields.extend(
                FieldOccurrence(
                    f"points.{point.point_id}.{name}", field.evidence.element_ids
                )
                for name, field in (
                    ("series", point.series),
