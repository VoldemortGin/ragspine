...
class BarVectorPaint:
    ...
    envelope_rule: Literal["pdf-solid-stroke-envelope-v1"] = (
        "pdf-solid-stroke-envelope-v1"
    )
    ...
    def intersects(self, bounds: Bounds) -> bool:
        if self.alpha == 0:
            return False
        return (
            self.envelope.intersects(bounds)
            if self.envelope
            else _intersects(self.bounds, bounds)
        )
...
def _vector_roles(
    ...
    for event in events:
        ...
        if event.kind == "fill":
            ...
            width = TypeAdapter(float).validate_python(
            ...
            stroke_envelope(
                ...
                dashes=TypeAdapter(str).validate_python(
                    event.payload["dashes"], strict=True
                ),
                region=roi,
            )
        matched = tuple(
...
def build_bar_source_paint_proof(
...
def prove_page_context(
    ...
    *,
    ...
    if (
        proof.source_page_spans_digest
        != sha256(repr(page.text.spans).encode()).hexdigest()
    ):
        raise ValueError("page_context_full_sidecar_differs_from_pdf")
    if not proof.glyphs or any(
        glyph.source_span_id != span.span_id for glyph in proof.glyphs
    ):
    ...
    return PageContextCitation(
        ...
        Confidence(
            None, "separate page-context source glyph/clip/alpha/occlusion proof"
        ),
    ), proof
