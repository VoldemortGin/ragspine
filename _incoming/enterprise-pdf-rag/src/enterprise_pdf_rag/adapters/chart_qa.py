...
class StoredChartResolver:
    ...
    def _resolve(self, pin: QueryPin) -> ChartContext:
        ...
        if (
            manifest.retrieval is None
            or manifest.retrieval.snapshot_id != pin.snapshot_id
        ):
            raise ChartQueryError(
        ...
        member = next(
            (item for item in plan.members if item.member_id == pin.member_id), None
        )
        if member is None:
            raise ChartQueryError(
        ...
        validated = resolve_chart_member(
            self.sources, self.outputs.assets, manifest.scope, member
        )
        return ChartContext(
            pin,
