...
from enterprise_pdf_rag.documents.ports import AssetStore, SourceExtractor


def verify_source(pdf: bytes, spec: DocumentSpec) -> None:
    if sha256(pdf).hexdigest() != spec.sha256:
        raise ValueError(
            "The selected source SHA-256 does not match; no other PDF is accepted"
        )
...
def ingest_document(
    ...
    if len(extracted.pages) != spec.page_count or tuple(
        ...
        raise ValueError(
            "Incomplete or ambiguous page coverage; manifest was not published"
        )
    for page in extracted.pages:
        if not all(
            isfinite(value) and value > 0 for value in (page.width, page.height)
        ):
            raise ValueError(
                f"Invalid source page geometry at page_index={page.page_index}"
            )
        for span in page.text_spans:
            if not all(
                isfinite(value)
                for value in (*span.bbox, *span.origin, span.size, *span.direction)
            ):
                raise ValueError(
                    f"Invalid source span geometry at page_index={page.page_index}"
                )
    ...
    region = extractor.extract_region(
        pdf, page_index=spec.focus_page_index, bbox=spec.focus_bbox
    )
    ...
    for page in extracted.pages:
        ...
        text = TextSidecar(
            "text-spans-v1", spec.sha256, page.page_index, page.text_spans
        )
        pages.append(
            PageRecord(
                page.page_index,
                ...
                store.put(
                    json.dumps(
                        asdict(text), ensure_ascii=False, sort_keys=True
                    ).encode(),
    ...
    region_text = TextSidecar(
        "text-spans-v1", spec.sha256, region.page_index, region.text_spans
    )
    record = RegionRecord(
        region.page_index,
        region.bbox,
        ...
        store.put(
            json.dumps(
                asdict(region_text), ensure_ascii=False, sort_keys=True
            ).encode(),
            media_type="application/json",
        ),
