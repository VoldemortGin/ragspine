...
def _numbers(text: str) -> tuple[Decimal, ...]:
    return tuple(
        Decimal(match.group().replace(",", "")) for match in _NUMBER.finditer(text)
    )
...
def validate_svg(svg: SvgArtifact) -> None:
    ...
    try:
        ...
        raise FigureError(
            FailureCode.INVALID_EVIDENCE, "SVG is not well-formed XML"
        ) from error
    if root.tag.rsplit("}", 1)[-1] != "svg":
        raise FigureError(
            FailureCode.INVALID_EVIDENCE, "Figure representation must be SVG"
        )
...
def evidence_elements(svg: SvgArtifact, evidence: Evidence) -> tuple[SvgElement, ...]:
    ...
    if any(element_id not in elements for element_id in evidence.element_ids):
        raise FigureError(
            FailureCode.INVALID_EVIDENCE, "Evidence refers to a missing SVG element"
        )
...
def _validate_claim(svg: SvgArtifact, chart: ChartIR, claim: DescriptionClaim) -> None:
    ...
    if not claim.text.strip() or claim.text.lstrip().startswith(
        ("{", "[", "<", "ChartIR(")
    ):
        raise FigureError(
            FailureCode.CONTENT_MISMATCH,
