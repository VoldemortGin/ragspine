...
def test_complete_source_assets_survive_a_store_reopen(tmp_path: object) -> None:
    ...
    spec = DocumentSpec(
        "aia.pdf", sha256(source).hexdigest(), 2, 1, (0.0, 0.0, 100.0, 100.0)
    )
...
def test_non_finite_geometry_cannot_advance_current_manifest(
    ...
    class InvalidGeometry:
        def extract_document(self, _pdf: bytes) -> DocumentExtraction:
            return DocumentExtraction(
                (PageExtraction(0, float("inf"), 100.0, 0, svg, ()),), "bad"
            )

        def extract_region(
            self, _pdf: bytes, *, page_index: int, bbox: Bounds
        ) -> RegionExtraction:
            return RegionExtraction(
                page_index, float("inf"), 100.0, 0, bbox, svg, svg, ()
            )
