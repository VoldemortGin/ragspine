...
def test_persisted_description_search_hydrates_typed_ir_without_granting_financial_qa(
    ...
    text = sources.put(
        json.dumps(asdict(sidecar)).encode(), media_type="application/json"
    )
    ...
    assert (
        context.description.text == "Agency 72%"
        and context.member.object_id == "object"
    )
    ...
    partition = PagePartition(
        "layout-v2", source_id, pdf.sha256, 0, "test", (item,), ()
    )
    ...
    page_record = PageProcessingRecord(
        0,
        StageOutcome(
            "canonical", "1" * 64, StageState.SUCCEEDED, "test", canonical_ref
        ),
        StageOutcome(
            "partition", "2" * 64, StageState.SUCCEEDED, "test", partition_ref
        ),
        (record,),
    )
    manifest = ProcessingManifest(
        "processing-v1", scope, "test", (page_record,), publication
    )
    ...
    assert (
        receipt_ref is not None and description_ref is not None and ir_ref is not None
    )
    receipt = TypeAdapter(LiteralQualification).validate_json(
        outputs.assets.get(receipt_ref)
    )
    description = TypeAdapter(ObjectDescription).validate_json(
        outputs.assets.get(description_ref)
    )
    ...
    if tampering == "producer":
        ...
        description = replace(
            ...
            confidence=Confidence(
                Decimal("1.0"), "independently verified financial relation"
            ),
        )
    ...
    ir_ref = outputs.assets.put(
        TypeAdapter(TextIR).dump_json(ir), media_type="application/json"
    )
    ...
    vector = TypeAdapter(RetrievalEmbedding).validate_json(
        outputs.assets.get(member.embedding)
    )
