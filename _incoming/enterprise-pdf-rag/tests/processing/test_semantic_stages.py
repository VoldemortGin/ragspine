...
@pytest.mark.parametrize(
    "valid_claim,valid_binding", [(True, True), (False, True), (True, False)]
)
def test_chart_failure_keeps_description_and_source_assets_without_qualification(
    ...
    def sender(url: str, *, api_key: str, payload: bytes, timeout: float) -> bytes:
        ...
        if len(requests) == 1:
            ...
            source = next(
                entry for entry in view["observations"] if entry["text"] == "Revenue"
            )
            content = {
                "schema_version": "figure-description-v1",
                "svg_digest": view["svg_digest"]
                if valid_binding or correction
                else "f" * 64,
                "claims": [
                    {
                        "text": "Revenue",
                        "evidence": {
                            "element_ids": [
                                source["id"] if valid_claim else "fabricated"
                            ],
                            "confidence": "high",
                        },
                        "series": None,
...
def test_explicit_chart_binding_correction_keeps_original_raw(tmp_path: Path) -> None:
    ...
    def sender(url: str, *, api_key: str, payload: bytes, timeout: float) -> bytes:
        ...
        if correction:
            view = json.loads(prompt.rsplit("\n", 2)[1])
        observation = next(
            row for row in view["observations"] if row["text"] == "Revenue"
        )
