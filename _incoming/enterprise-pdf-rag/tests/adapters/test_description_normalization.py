...
import pytest
from tests.figures.test_chart_qa_displayed import displayed_context
...
def test_exact_duplicate_repair_restores_raw_claim_without_changing_its_semantics() -> (
    ...
    assert all(
        claim.evidence.verification is Verification.PENDING
    ...
    assert result.normalized.claims[0].model_dump(
        exclude={"evidence"}
    ) == original.claims[0].model_dump(exclude={"evidence"})
    assert (
        result.normalized.claims[0].evidence.confidence
        == original.claims[0].evidence.confidence
    )
    ...
    assert (
        result.receipt.output_description_artifact_id == result.description.artifact_id
    )
    ...
    again = normalize_description_evidence(
        svg=svg, raw_json=raw_json, previous=previous
    )
    assert again == result


def test_an_unrelated_previous_typed_artifact_cannot_be_used_as_repair_lineage() -> (
    None
):
    svg, raw_json, previous = independent_branch()
    wrong = replace(
...
def test_decimal_precision_and_confidence_are_preserved_not_corrected() -> None:
    ...
    first = dto.claims[0].model_copy(
        update={
            "value": "8.20",
            "evidence": dto.claims[0].evidence.model_copy(
                update={"confidence": "0.25"}
            ),
        }
    )
    dto = dto.model_copy(update={"claims": (first, *dto.claims[1:])})
