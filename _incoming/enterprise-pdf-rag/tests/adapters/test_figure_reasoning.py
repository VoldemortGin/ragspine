...
def test_model_view_binds_saved_svg_and_excludes_partially_intersecting_observations() -> (
    None
):
    ...
    assert all(
        element.source_span_id == "value-span" for element in result.svg.elements
    )
    assert {element.text for element in result.svg.elements} == {"72%", "72", "%"}
    assert all(
        element.evidence_kind.value == "source_text_observation"
        for element in result.svg.elements
    )
...
def test_whitespace_spans_do_not_invent_reversed_unicode_ranges(blank: str) -> None:
    ...
    assert {
        (element.text, element.text_range) for element in prepared.svg.elements
    } == {("72%", (1, 4)), ("72", (1, 3)), ("%", (3, 4))}
    validate_svg(prepared.svg)


def test_page_context_is_separately_bound_and_empty_context_keeps_the_old_view() -> (
    None
):
    ...
    bare = prepare_figure(
        page=page, native_svg=SVG, bbox=(0.0, 0.0, 100.0, 40.0), region_id="fx"
    )
    ...
    assert (
        context.source.document_sha256 == source_sha and context.source.page_index == 19
    )
    assert context.scope == "page_context"
    assert all(
        element.source_span_id != "footer" for element in contextual.svg.elements
    )
