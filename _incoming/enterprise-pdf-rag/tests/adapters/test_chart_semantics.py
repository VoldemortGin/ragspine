...
def test_independent_branches_bind_the_same_svg_and_do_not_promote_model_confidence(
        ...
        "claims": [
            {
                ...
                "evidence": {
                    "element_ids": [
                        ids[text] for text in ("VONB", "Agency", "72", "%", "1H26")
                    ],
    ...
    client = JsonCompletionClient(
        config, cache_dir=tmp_path, max_live_calls=2, sender=sender
    )
    ...
    assert (
        chart_result.chart.binding
        == description_result.description.binding
    ...
    assert (
        chart_result.chart.points[0].value.evidence.verification is Verification.PENDING
    )
    ...
    if confidence == "high":
        ...
        assert (
            "ordinal=high"
            in chart_result.chart.points[0].value.evidence.confidence.method
        )
    elif confidence == "source_text_observation":
        assert chart_result.chart.points[0].value.evidence.confidence.score is None
        assert (
            description_result.description.claims[0].evidence.confidence.score is None
        )
    assert (
        chart_result.completion.output_digest
        != description_result.completion.output_digest
    )
...
def test_model_cannot_rebind_observations_to_another_svg(
    ...
    with pytest.raises(
        JsonCompletionError, match="model_svg_binding_mismatch"
    ) as raised:
        if branch == "chart":
    ...
    assert (
        getattr(raised.value, "output_digest", None) == sha256(raw.encode()).hexdigest()
    )
