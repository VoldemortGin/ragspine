...
def test_cli_demo_writes_reviewable_artifacts(
    ...
    async def exercise() -> None:
        async with AsyncClient(
            ...
            resolved = await client.post(
                "/v1/context", json={"snapshot_id": "api-v1", "hit": hit}
            )
            assert resolved.status_code == 200
            assert (
                resolved.json()["context"]["chart_ir"]["binding"]["svg_digest"]
                == hit["svg_digest"]
            )
            wrong = await client.post(
                "/v1/context", json={"snapshot_id": "other", "hit": hit}
            )
            assert wrong.status_code == 409
            empty = await client.post(
                "/v1/search", json={"snapshot_id": "api-v1", "query": "   "}
            )
