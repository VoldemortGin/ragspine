...
import pytest
from tests.adapters.test_chart_qa_store import published_chart, stored_question
...
def test_source_numeric_draft_preserves_old_refusal_vectors_and_current(
    ...
    answer = ChartQAService(
        StoredChartResolver(
            sources, outputs, processing_id=release.current.processing_id
        )
...
def test_promotion_cannot_reuse_vector_for_different_description_or_lineage(
    ...
    new = replace(
        ...
        if change == "embedding_dimensions"
        else old.embedding_dimensions,
    )
...
def test_validated_draft_preserves_current_pointer_and_missing_source_fails(
    ...
    ).read_text().strip() == pin.processing_id
