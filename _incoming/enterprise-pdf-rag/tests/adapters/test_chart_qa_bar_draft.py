...
def test_source_qualified_bar_draft_reuses_prior_vectors_and_is_idempotent(
    ...
    assert embedder.descriptions == [
        "Expense Ratio for 1H21: 15 %. Expense Ratio for 1H23: 6 %."
    ]
    ...
    assert all(
        vectors[entry.member_id] == entry.vector for entry in prior_index.entries
    )
    ...
    assert (
        answer.description_normalization is not None and len(answer.page_context) == 1
    )
...
def test_stored_v2_http_and_review_distinguish_lookup_from_numeric_relations(
    ...
    app = FastAPI()
    app.include_router(
        create_processing_router(sources, outputs, release.current.processing_id)
    )
    ...
    async def exercise() -> None:
        async with AsyncClient(
            ...
            crossed = await client.post(
                "/v1/queries", json={**payload, "snapshot_id": "f" * 64}
            )
