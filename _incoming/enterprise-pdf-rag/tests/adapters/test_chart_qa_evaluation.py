...
import pytest
from tests.figures.test_chart_qa import PinnedResolver, qualified_context, question
...
from enterprise_pdf_rag.figures.chart_qa.models import Operation, PointSelector
...
def _anchor(
    gold: ChartQaGold, bbox: tuple[float, float, float, float]
) -> dict[str, Any]:
    return {
...
def _citation(
    ...
    return {
        ...
        else f"points.{point_id}.{field_name}",
    ...
    if case.request.operation == "percentage_point_difference":
        ...
        for item in inputs:
            citation = next(
                ...
                if entry["field_path"].endswith(".value")
            )
...
def test_wrong_number_and_wrong_source_occurrence_fail_separate_metrics() -> None:
    ...
    result = next(
        ...
        if item["case_id"] == "lookup-agency-72"
    )
...
def test_bad_citation_fails_citation_gate_without_rewriting_answer_precision() -> None:
    ...
    result = next(
        item
        for item in observations["results"]
        if item["case_id"] == "lookup-partnerships-28"
    )
...
def test_difference_receipt_must_preserve_operand_order_even_if_output_matches() -> (
    None
):
    gold = load_gold(GOLD_PATH.read_bytes())
    observations = _perfect_observations(gold)
...
def test_explicit_paths_write_one_immutable_content_addressed_bundle(
    ...
    observations_payload = json.dumps(
        _perfect_observations(gold), separators=(",", ":")
    ).encode()
