...
from enterprise_pdf_rag.adapters.providers import ProviderRequestError, load_llm_config


def _response(prepared: PreparedFigure, digest: str) -> bytes:
    element = next(
        element for element in prepared.svg.elements if element.text == "VONB"
    )
    content = {
...
def test_explicit_binding_correction_preserves_old_result_and_reuses_new_attempt(
    ...
    def sender(url: str, *, api_key: str, payload: bytes, timeout: float) -> bytes:
        ...
        return _response(
            prepared, "0" * 64 if len(payloads) == 1 else prepared.svg.digest
        )
...
def test_correction_requires_the_matching_cached_binding_failure(
    ...
    def sender(url: str, *, api_key: str, payload: bytes, timeout: float) -> bytes:
        ...
        return _response(
            prepared, prepared.svg.digest if prior == "already-correct" else "0" * 64
        )
    ...
    with pytest.raises(
        ...
        ModelDescriptionGenerator(client, prepared, correction_of=reference).infer(
            prepared.svg
        )
...
def test_failed_correction_is_not_retried_by_a_client_with_retry_enabled(
    ...
    def sender(url: str, *, api_key: str, payload: bytes, timeout: float) -> bytes:
        ...
        raise ProviderRequestError(
            "bounded correction failed", status=429, category="http"
        )
