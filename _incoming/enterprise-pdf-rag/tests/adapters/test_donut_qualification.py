...
def sample(
    *, extra_category: bool = False, geometry: str = "supported"
    ...
    number_dx = (
        -4.2
        if geometry == "near-boundary"
        else 25.0
        if geometry == "center-hole"
        else 0.0
    )
...
def test_adjacent_label_cannot_replace_a_category_even_when_both_models_agree() -> None:
    ...
    category = TextField(
        "Other", replace(original.evidence, element_ids=(other.element_id,))
    )
    chart = replace(
        chart, points=(replace(chart.points[0], category=category), chart.points[1])
    )
...
def test_unsupported_source_geometry_or_unsafe_label_location_is_unavailable(
    ...
    with pytest.raises(
        FigureError, match=r"sector|scope_ambiguous|source_transform|source_paint"
    ):
        DonutQualification(prepared).qualify_pair(prepared.svg, chart, description)


def test_swapping_categories_in_both_branches_cannot_qualify_the_wrong_source_relation() -> (
    None
):
    prepared, chart, description = sample()
    ...
    claims = tuple(
        ...
        for claim, old, new in zip(
            description.claims, chart.points, swapped.points, strict=True
        )
    )
...
def test_correct_values_do_not_qualify_an_added_market_or_causal_conclusion() -> None:
    ...
    claim = replace(
        ...
        text=description.claims[0].text
        + " Agency leads the market because of policy changes.",
    )
...
def test_qualification_cannot_read_paths_from_a_crop_other_than_the_model_source() -> (
    None
):
    prepared, chart, description = sample()
    prepared = replace(
        prepared, crop_svg=prepared.crop_svg.replace("#d31145", "#abcdef")
    )
            ...
            points=(
                replace(
                    ...
                    value=replace(
                        first.value,
                        evidence=replace(
                            first.value.evidence, verification=Verification.REJECTED
                        ),
                    ),
                ),
                second,
        ...
        description = replace(
            ...
            claims=(
                replace(
                    first_claim,
                    evidence=replace(
                        first_claim.evidence, verification=Verification.REJECTED
                    ),
                ),
                second_claim,
            ),
