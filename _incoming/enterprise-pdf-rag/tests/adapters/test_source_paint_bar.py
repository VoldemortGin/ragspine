...
from tests.adapters.bar_source_fixture import bar_source
from tests.adapters.test_source_paint import glyph_source
...
def test_page_context_gets_its_own_full_source_and_visible_glyph_proof() -> None:
    ...
    span = next(
        span for span in source.page.text.spans if span.text == ACTUAL_FX_CONTEXT
    )
    native = (
...
def test_invisible_or_later_covered_footer_cannot_supply_page_context(
    ...
    with pytest.raises(
        ValueError, match=r"unsupported_source_text_mode|source_glyph_occluded"
    ):
        prove_page_context(
            source.pdf,
            page=source.page,
