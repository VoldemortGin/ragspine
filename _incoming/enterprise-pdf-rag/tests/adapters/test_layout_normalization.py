...
def _page(
    *, source: str = "b" * 64, page_index: int = 0, spans: tuple[TextSpan, ...]
) -> PageInput:
    svg = b"<svg/>"
    return PageInput(
        "a" * 64,
...
def test_source_occurrence_rounding_expands_roi_without_overwriting_model_geometry() -> (
    None
):
    span = TextSpan(
        "title",
    ...
    assert normalized.objects[0].normalization == (
        "source-span-outward-containment-v1",
    )
...
def test_reviewed_p18_chart_bounds_expand_model_roi_and_lock_first_donut() -> None:
    ...
    first = next(
        ...
        if entry["object_id"] == "aia-p018-distribution-mix-v1"
    )
    right = next(
        entry for entry in entries if entry["object_id"] == "aia-p018-chart-03-v1"
    )
    ...
    spans = tuple(
        TextSpan(span_id, span_id, bbox) for span_id, bbox in span_boxes.items()
    )
    page = _page(
...
def test_reviewed_p18_kpi_cards_become_pending_groups_with_raw_kind_retained() -> None:
    ...
    assert (
        tuple(item.kind for item in normalized.objects[:4]) == (ObjectKind.GROUP,) * 4
    )
    assert all(
...
def test_reviewed_p20_sensitivity_panel_becomes_pending_group() -> None:
    ...
    normalized = normalize_partition(
        page=page, partition=_partition(page, (raw_object,))
    )
...
def test_reviewed_p5_rasters_and_connector_become_pending_visual_children() -> None:
    ...
    assert (
        diagram.source_span_ids == first.source_span_ids == second.source_span_ids == ()
    )
    assert all(
