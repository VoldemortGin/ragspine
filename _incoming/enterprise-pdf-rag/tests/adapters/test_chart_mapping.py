...
def _point(prepared: PreparedFigure, point_id: str = "good") -> ChartPointDTO:
    return ChartPointDTO(
        ...
        value=NumericDTO(
            value="72", kind="explicit", evidence=_field(prepared, "72").evidence
        ),
    )
...
def _infer(
    tmp_path: Path, prepared: PreparedFigure, dto: ChartObservationsDTO
) -> ChartInference:
    sends: list[bytes] = []
...
def test_one_missing_occurrence_omits_only_affected_members_without_borrowing_evidence(
    ...
    dto = _dto(prepared, good, bad).model_copy(
        update={"axes": (axis,), "marks": (mark,)}
    )
    ...
    assert any(
        "points.bad.unit:missing_source_evidence" in item for item in result.diagnostics
    )
    assert any(
        "axes.y.label:missing_source_evidence" in item for item in result.diagnostics
    )
    assert any(
        "marks.bad-mark:unmapped_point_relation" in item for item in result.diagnostics
    )
...
def test_uninterpretable_numeric_literal_is_unavailable_without_losing_the_point(
    ...
    point = _point(prepared).model_copy(
        update={
            "value": NumericDTO(
                value=literal, kind="explicit", evidence=observed.evidence
            ),
            "unit": TextFieldDTO(text=unit, evidence=observed.evidence),
        }
    )
