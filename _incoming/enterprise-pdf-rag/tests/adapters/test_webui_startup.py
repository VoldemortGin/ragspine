...
def _project(tmp_path: Path) -> Path:
    ...
    shutil.copyfile(Path("scripts/webui_preview.py"), scripts / "webui_preview.py")
    return root
...
def test_start_requires_saved_processing_without_starting_services(
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(root / "scripts/webui_preview.py"),
            "start",
            "--require-processing",
        ],
        cwd=root,
...
def test_start_refuses_a_pid_record_for_an_unrelated_live_process(
    ...
    result = subprocess.run(
        [sys.executable, str(root / "scripts/webui_preview.py"), "start"],
        cwd=root,
        env={"PATH": os.defpath, "APP_ROOT_DIR": str(root)},
        capture_output=True,
        text=True,
