...
def authored_pdf(
    path: Path, *, page_count: int, label: str, embedded_font: bool = False
) -> Path:
        ...
        for number in range(page_count):
            ...
            if embedded_font:
                ...
                cast(_FontInsertionPage, page).insert_font(
                    fontname=fontname,
                    fontbuffer=(
                        ROOT_DIR / "tests/fixtures/authored-donut-ascii.ttf"
                    ).read_bytes(),
                )
            page.insert_text(
                (20, 40), f"{label} page {number + 1}", fontsize=12, fontname=fontname
            )
...
def test_public_api_accepts_pages_beyond_twenty_and_saves_full_source(
    ...
    assert result.source_store == str(
        tmp_path / "output" / result.source_sha256 / "source"
    )
    ...
    assert all(
        not page.objects and page.raw_partition is None for page in manifest.pages
    )
    ...
    canonical = TypeAdapter(CanonicalPage).validate_json(
        outputs.assets.get(canonical_ref)
    )
...
def test_script_and_api_isolate_documents_and_resume_without_source_extraction(
    ...
    completed = subprocess.run(
        [sys.executable, str(ROOT_DIR / "scripts" / "ingest.py"), "--pdf", str(first)],
        cwd=tmp_path,
        env=environment,
        capture_output=True,
        text=True,
...
def test_invalid_selected_pages_do_not_write_outputs(
...
def test_unsupported_source_coordinates_fail_explicitly_without_a_draft(
    ...
    with pytest.raises(
        ValueError, match="Page index 0 extraction failed: Rotated pages"
    ):
...
def test_explicit_stages_share_budget_preserve_partial_branches_and_replay(
    ...
    pdf = authored_pdf(
        tmp_path / "metric.pdf", page_count=1, label="Metric", embedded_font=True
    )
    for key, value in {
    ...
    monkeypatch.setattr(
        "enterprise_pdf_rag.adapters.json_completion._send_once", sender
    )
    output = tmp_path / "output"
    ...
    assert not calls
    partial = ingest_pdf(
        pdf=pdf, output_dir=output, stage="semantics", max_live_calls=2
    )
    ...
    complete = ingest_pdf(
        pdf=pdf, output_dir=output, stage="semantics", max_live_calls=1
    )
...
def test_script_reports_invalid_range_without_traceback_or_output(
    ...
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT_DIR / "scripts" / "ingest.py"),
            "--pdf",
            str(pdf),
            "--pages",
            "2",
    ...
    assert (
        "Pages must exist" in completed.stderr and "Traceback" not in completed.stderr
    )
