...
def test_lookup_preserves_explicit_display_and_each_field_occurrence() -> None:
    ...
    assert result.answer.confidence.method.startswith(
        "source-qualified explicit percentage"
    )
    ...
    for citation in claim.citations:
        ...
        for occurrence in citation.occurrences:
            ...
            assert (
                occurrence.anchor.document_sha256 == context.svg.source.document_sha256
            )
...
def test_difference_is_ordered_decimal_calculation_not_a_source_literal() -> None:
    ...
    query = replace(
        ...
        points=tuple(
            PointSelector(p.point_id, p.category.text) for p in context.chart.points
        ),
    )
    ...
    assert result.answer.confidence.score is None
    assert result.answer.confidence.method.startswith(
        "deterministic Decimal subtraction"
    )
...
def test_wrong_semantic_request_abstains(
    field: str, value: str, reason: RefusalReason
) -> None:
    context = qualified_context()
...
def test_receipt_wrong_occurrence_and_pending_field_fail_closed() -> None:
    ...
    with pytest.raises(ChartQueryError, match="every exact chart field"):
        ChartQAService(
            PinnedResolver(replace(context, qualification=bad_receipt))
        ...
        ChartQAService(
            PinnedResolver(
                replace(
                    context,
                    chart=replace(
                        context.chart, points=(changed, *context.chart.points[1:])
                    ),
                )
            )
        ).answer(question(context))
...
def fractional_context(left: str, right: str) -> ChartContext:
    ...
    spans = tuple(
        replace(span, text=replacements.get(span.text, span.text))
    ...
    def field(value: TextField | None) -> TextField | None:
        return (
            None if value is None else replace(value, evidence=evidence(value.evidence))
        )
    ...
    points = tuple(
        replace(
            ...
            category=replace(
                point.category, evidence=evidence(point.category.evidence)
            ),
...
def test_qualified_numeric_substring_cannot_hide_comparison_or_estimate(
    ...
    changed = tuple(
        ...
        if element == whole
            ...
            element, text_range=(element.text_range[0] + 1, element.text_range[1] + 1)
        )
        if element.source_span_id == whole.source_span_id
