...
def _build_data_sheet(wb: Workbook) -> str:
    ...
    for metric, (vals, fmt, rgb, tags) in data.items():
        a = ws.cell(row=row, column=1, value=metric)
    ...
    a = ws.cell(row=row, column=1, value="ROE")
...
def _expected_clusters(sheet: str) -> list[dict]:
    ...
    for key, t in _cell_truth.items():
        if t["sheet"] != sheet:
            continue
    ...
    clusters = [{"rgb": rgb, "cell_refs": sorted(refs), "count": len(refs)}
                for rgb, refs in by_rgb.items()]
...
def _build_merged_header_sheet(wb: Workbook) -> str:
    sheet = "MergedHeader"
    ws = wb.create_sheet(sheet)
...
def _build_transposed_sheet(wb: Workbook) -> str:
    sheet = "Transposed"
    ws = wb.create_sheet(sheet)
...
def _build_cf_sheet(wb: Workbook) -> str:
    sheet = "CondFormat"
    ws = wb.create_sheet(sheet)
