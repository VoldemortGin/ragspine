...
def main(argv: list[str] | None = None) -> int:
    ...
    for f in report.files:
        detail = f"chunks={f.n_chunks}"
        if f.n_skipped_pages:
            detail += f", 跳过扫描页={f.n_skipped_pages}"
        if f.error:
            detail += f", error={f.error}"
        print(f"{prefix}{f.doc_id}: {f.status} ({detail})")
        for w in f.warnings:
            print(f"    警告: {w}")

    counts = report.counts()
    total_chunks = sum(f.n_chunks for f in report.files)
    total_skipped_pages = sum(f.n_skipped_pages for f in report.files)
    print(
