...
export class ApiError extends Error {
  ...
  constructor(message: string, status: number, type: string, requestId?: string, body?: unknown) {
    super(message);
    this.name = 'ApiError';
    this.status = status;
    this.type = type;
...
export function compileWorkflow(yaml: string, foldAnswerQuestion = true): Promise<CompileResponse> {
...
export async function fetchTopology(
  ...
  const res = await request<TopologyResponse>(`/v1/topology?scope=${encodeURIComponent(scope)}`);
...
export function submitStructuredIngest(req: StructuredIngestRequest): Promise<JobRef> {
  return postJson<JobRef>('/v1/ingest/structured/jobs', req);
}

export function submitNarrativeIngest(req: NarrativeIngestRequest): Promise<JobRef> {
...
export function convertN8n(
  ...
  return postJson<N8nConvertResponse>('/v1/n8n/convert', { direction, workflow });
}
