 ...
 * contain node types outside the 17 palette types. Those must be preserved
...
export interface StudioNode {
  ...
  type: string;
  position: XY;
  /** Container id (iteration/loop), derived from data.iteration_id or data.loop_id on import. */
  parentId?: string;
  data: StudioNodeData;
...
export interface HttpAuthorizationConfig {
...
export interface HttpAuthorization {
  ...
  config?: HttpAuthorizationConfig | null;
  [key: string]: unknown;
}

export interface HttpBodyItem {
  id?: string;
  key?: string;
  type?: 'text' | 'file';
  value?: string;
...
export interface HttpBody {
  ...
  /** New DSLs carry HttpBodyItem[]; a plain string is the legacy shape. */
  data?: string | HttpBodyItem[];
  [key: string]: unknown;
}

export interface HttpRequestNodeData extends StudioNodeData {
  method?: 'get' | 'post' | 'head' | 'patch' | 'put' | 'delete';
  /** May contain {{#node.var#}} interpolation. */
  url?: string;
  /** Multiline "Key: Value" lines. */
  headers?: string;
  /** Same "Key: Value" line format as headers. */
  params?: string;
...
export interface VariableAggregatorNodeData extends StudioNodeData {
  output_type?: string; // default "any"
  variables?: ValueSelector[];
  advanced_settings?: VariableAggregatorAdvancedSettings;
}

export interface AssignerItem {
  variable_selector?: ValueSelector;
...
export interface AssignerNodeData extends StudioNodeData {
  version?: string; // '2'
  items?: AssignerItem[];
}

export interface DocumentExtractorNodeData extends StudioNodeData {
  /** Points at a file / array[file] variable. */
  variable_selector?: ValueSelector;
  is_array_file?: boolean; // default false
}
...
export interface LoopVariable {
  id?: string;
  label?: string;
  var_type?: string; // string | number | object | boolean | array[...]
  value_type?: 'variable' | 'constant';
  value?: unknown;
  [key: string]: unknown;
}

export interface LoopNodeData extends StudioNodeData {
  loop_count?: number; // default 10
  break_conditions?: LoopBreakCondition[];
  logical_operator?: 'and' | 'or'; // default 'and'
  /** Points at the loop-start node inside the container (not generated by the studio). */
  start_node_id?: string;
  loop_variables?: LoopVariable[];
  loop_id?: string; // present on CHILD nodes, not on the container
}
...
export interface NodeTypeDefinition {
  /** One of the 17 DifyNodeType values, or the raw type for the fallback def. */
  type: string;
  label: string;
  description: string;
  ...
  accent: string;
  /** True only for the container types iteration and loop (rendered as a container/group node). */
  isContainer: boolean;
