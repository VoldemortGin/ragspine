...
本版保留 [v0.1](PRD-v0.1.md)，补充 figure 的结构化 SVG、同源 ChartIR/自然语言双路，以及描述检索后加载 ChartIR 的完整契约。

**本次核心修订：** PDF figure → 可审阅的结构化 SVG → 同源并行生成 ChartIR 与自然语言描述 → 仅描述 embedding → 命中后加载同 release 的 ChartIR → 验证、计算并构建 ReasoningView。
PNG 仅辅助比对；ChartIR 和原始 SVG 不进入 embedding；缺 ChartIR 或版本不匹配时不能只靠描述回答。详细契约集中在 §4.4。
...
figure 仅存一张 PNG 不算完成：需要可审阅的 HTML 内嵌 SVG 或结构化 SVG，再从同一份 SVG 分别生成 ChartIR 和精准自然语言描述。
figure 的向量检索只编码自然语言描述；命中后读取对应 ChartIR 与原始图形证据，不能把摘要本身当作精确问答依据。
...
| Enrichment | 表头解释、ChartIR、TextDescription、领域映射、模型观察候选、推断、验证和人工修正 | ChartIR 与描述直接读取同一 SVG；每项引用 evidence，不覆盖 Canonical |
| Projection | Figure Representation（结构化 SVG/内嵌 SVG 的 HTML）、Markdown、表格行、财务与 viewer 视图 | 可重建、有来源的表示；图形表示不等于图表数值已经恢复 |
| Retrieval | 词法、向量、结构化检索索引及 evidence pointers | figure 只对 TextDescription 做 embedding；禁止编码 ChartIR 或原始 SVG |
| ReasoningView | 同一固定 release 下，经资格验证后供回答或计算使用的证据集合 | 不是模型隐藏思维链；不将模型回答回写成事实 |

```mermaid
flowchart TD
    R[Raw Assets] --> C[Canonical IR]
    C --> S[Projection Figure SVG]
    S --> G[Enrichment ChartIR]
    S --> D[Enrichment TextDescription]
    C --> E[其他 Enrichment]
    C --> P[其他 Projection]
    E --> P
    C --> I[词法与结构检索]
...
图中 figure 双路及描述索引在发布前还须通过 §4.4 的配对校验，箭头不表示绕过校验直接索引。
Figure Representation 从 Canonical 观察及有来源的重建结果产生；后续 ChartIR/描述属于不同 Enrichment artifacts，不能形成循环依赖。
Projection 可以作为索引构建输入，但只有经批准的文本进入 embedding；显示模板变化仅在索引确实依赖该投影时触发重建。
...
原生文字与矢量观察优先；难区域按质量策略调用 OCR/VLM 增强端口，输入限于有来源的页面、裁剪图或 Figure SVG 资产。
不能将原 PDF 交给替代 PDF parser，也不能让模型 response 直接创建、覆盖或发布 Canonical。
增强输出先作为带证据的候选进入 Enrichment，经严格校验和资格判断后才能参与投影、检索或回答；SVG 重建候选同样不能直接覆盖 Canonical。
...
23. 作为审核者，我希望 figure 有保留图形、文字、颜色、图例与几何关系的可审阅 SVG，以便核对其与 PDF 原区域的差异。
24. 作为查询者，我希望用自然语言描述找到图表后，系统加载同版本 ChartIR，以便据完整坐标轴、单位、系列和字段证据回答。
25. 作为维护者，我希望 ChartIR 与描述来自同一 SVG 且可以分别重建，以便更换描述模型时复用有效 ChartIR，更换 ChartIR 提取器时复用有效描述文本与向量。
26. 作为审核者，我希望描述中的每个数值和系列归属都能验证，以免流畅描述把估计读数写成精确值或把数据归到错误系列。
...
| FigureRepresentation | 从来源观察生成可审阅的结构化 SVG/HTML 与 element-to-source 映射 | 区分 vector preserve 与 raster reconstruction；不能用 PNG 包装冒充结构化 SVG |
| Enricher / LLM | 对指定 evidence 执行增强、ChartIR 提取或直接从 SVG 生成描述 | 输入输出验证；模型配置显式注入；禁止直接发布事实 |
| Embedding | 对经批准的检索文本编码 | figure 仅接受 TextDescription；拒绝 ChartIR、其序列化文本及原始 SVG |
| FigureContextResolver | 在指定 release 内由 description references 加载配对 ChartIR 和 SVG 证据 | 校验版本、同源绑定、字段资格与权限；不能 fallback 到仅摘要 |
...
分类观察同样记录 producer、方法和状态；pdfspine 仅提供 figure 时可保存为 Image/Group，保留 figure anchors 与 source primitive observations，再经 Figure SVG 生成 ChartIR，不为凑类型伪造 Canonical Chart 事实。
...
### 4.4 Figure SVG、ChartIR 与描述检索

**图形表示。** PDF figure 经 pdfspine 取得区域及源观察，产出可审阅的 HTML 内嵌 SVG 或独立结构化 SVG。
SVG 必须保存可获得的图形、文字、颜色、图例符号、几何、分组、裁剪与变换，并赋予可追溯的 element IDs。
可选 PNG 只用于原件比对、复核或说明缺失；只有 `<image>` 包裹 PNG 的 SVG 不算结构化输出，更不能算 figure ingestion 完成。
vector preserve 尽量保留源矢量；raster/glyph reconstruction 是另一种有推断来源的过程，逐项记录重建方法、依据、置信度、视觉差异和未解内容，不能标作无损恢复。
重建候选先成为 EnrichedFigureArtifact，再生成新的版本化 SVG Projection；原观察和源 SVG 留存，不能覆盖或回流修改源 artifact。各版本依赖形成无环 DAG。
视觉保真只证明可视表示的一部分，不能证明原生图表数据已经恢复；新增文字或数字必须能回到 PDF 原区域，无依据内容保持 unknown/unavailable。

当前 pdfspine 的 `Page.get_svg_image()` 已有真实 path/clip/CTM 导出，但尚不是本阶段的完整实现：嵌入字体通常转 glyph path，`text_as_path=False` 当前未生效，没有公开的 figure clip 参数，部分 dash/shading/tiling/blend/mask 表达不完整。
P5 必须补齐或明确隔离 figure 区域导出、可读文字与图形 element 映射、完整性报告，以及对 PDF 原视图的视觉与语义校验；未通过的 SVG 不得作为合格双路输入。

**同 SVG 双路。** 两支分别直接读取同一个 `svg_artifact_id` 与 content digest，且绑定相同 `source_revision_id`、`figure_id`，在依赖上并行。
第一支提取 ChartIR；第二支由 LLM 直接据 SVG 生成精准自然语言 TextDescription。不得用“ChartIR 序列化 → 唯一摘要”替换第二支。
两支都属于 Enrichment，SVG 是 Projection 中的 Figure Representation；模型输出不得写回 Canonical。

```mermaid
flowchart TD
    PDF[PDF figure] --> PS[pdfspine 区域与源观察]
    PS --> SVG[有来源的结构化 SVG / HTML 内嵌 SVG]
    SVG --> CIR[ChartIR 提取]
    SVG --> DESC[LLM 直接生成 TextDescription]
    CIR --> CHECK[逐字段与逐 claim 验证 / 同源配对]
    DESC --> CHECK
    SVG --> CHECK
    PS --> CHECK
    CHECK --> TEXT[只取合格描述文本]
    TEXT --> EMB[Embedding 与描述索引]
    CHECK --> BUNDLE[FigureBundle / Release Manifest]
    EMB --> BUNDLE
    QUERY[查询] --> HIT[描述检索命中]
    EMB --> HIT
    HIT --> LOAD[按固定 release 加载 FigureBundle]
    BUNDLE --> LOAD
    LOAD --> CONTEXT[ChartIR 字段 + 必要 SVG / PDF 证据]
    CONTEXT --> RV[资格验证与确定性计算 → ReasoningView]
```

| 产物 | 必备内容与关联 | 用途与限制 |
| --- | --- | --- |
| Figure SVG | figure/source revision、SVG ID/digest、element IDs、坐标变换、source primitive/region 映射、完整性与重建来源 | 两支共享的不可变输入；不直接 embedding |
| ChartIR | SVG ID/digest、figure/source revision、grammar、字段状态、值来源、验证记录 | 回填问答结构上下文；不 embedding，不把 JSON/字段拼串绕送 embedding |
| TextDescription / slice | description ID、文本 digest、figure/source revision、SVG ID/digest、claim-to-element 证据 | 可有多段，只对经验证的自然语言文本 embedding；不依赖先完成 ChartIR 才生成 |
| FigureBundle | 同一 release 中的 figure/source revision、SVG ID/digest、ChartIR artifact ID、description IDs、配对验证、描述索引 references | 将每段描述解析为该 release 的唯一 ChartIR；不得使用另一个 release 的关联 |

ChartIR 基于 GrammarBasedObservations，包含 axes、scales、ticks、legend、marks、labels、series、encoding/relations、units 与逐值 provenance；字段引用 SVG elements，再追溯 PDF 原区域。
类别轴、时间轴、对数轴、双轴、堆叠及图例到系列的映射不能互相套用；不支持的 grammar 显式 unknown。
描述必须逐 claim 验证，数值、单位、趋势和系列归属绑定 SVG elements 与适用的 ChartIR field/series/point IDs；无法得到精确值时不能用自然语言编出精确值。
两路交叉对照可以发现遗漏或冲突，但同源一致不是两个独立真相证明，必须回到 PDF 原区域验证；冲突不得自动用一支覆盖另一支，也不能凭更高 confidence 放行。
cross-validation 与 FigureBundle 是独立版本化产物：描述文本的不可变 payload 主要绑定 figure 与 SVG，配对结果记录其在该 release 对应的 ChartIR fields。换 ChartIR 后须重新配对，不必无端重写描述文本。
每段检索记录都能解析出 `description_id → figure_id / svg_artifact_id / chart_ir_artifact_id / release_id`，有需要时再细化到 series/point；该关联来自 manifest，不靠检索时猜测或读取 latest。
只有 SVG 资格、两路内容验证、配对验证和描述 embedding/index readiness 全部通过，FigureBundle 才能随 release 原子发布。缺路或失配不允许发布仅描述的 figure。
unknown 字段是完整契约中的显式缺失状态，不等于缺少 ChartIR 产物；它可以支持经验证的有限描述，但不能支持所缺字段的精确答案。

**命中后的结构上下文。** 查询先检索 TextDescription，再通过 pinned release 解析 FigureBundle，加载 ChartIR 与必要 SVG 证据；随后执行结构、来源、权限及字段资格校验和确定性计算，才形成 ReasoningView。
不能直接拿 description/summary 代替上述过程回答；缺 ChartIR、配对失效或跨 release 引用时明确失败，不静默 fallback 到仅摘要。
同一 figure 的多个描述命中先去重；按问题和预算选择相关系列/数据点，但必须保留影响解释的全局 axes、scales、units、legend 及对应系列证据。预算不足时扩大受控读取、复核或拒答，不能截掉语义前提继续算数。
最终引用链为 claim/计算输入 → ChartIR field → SVG element → PDF source revision/page/region；单独的描述命中分数不授予证据资格。
...
遇到未知图种，通过已配对的有限描述命中后读取 ChartIR 的 unknown 状态和源图证据，回答“无法获得精确读数”；不能悄悄按普通柱状图解释。
...
| figure / SVG element / ChartIR field ID | figure 绑定源区域，SVG element 绑定 SVG artifact，series/point/field 绑定 ChartIR artifact；跨版本通过显式配对关联 |
...
| 显示模板（SVG 内容未变化） | 对应 Projection；仅其真实索引消费者继续失效 | Canonical、未受影响的 figure 双路及检索索引 |
| Figure SVG 内容/重建版本 | 依赖它的 ChartIR、描述、配对校验、描述 embedding/index 与 FigureBundle | 原始 PDF、未受影响的其他 figure |
| 描述模型/提示词 | 该描述、其 embedding/index、配对验证与 FigureBundle | 同源且仍有效的 SVG、ChartIR |
| ChartIR 提取器/解释版本 | ChartIR、配对验证、FigureBundle 及结构问答资格 | 同源且仍有效的描述文本与 embedding；若校验发现冲突则隔离，不无端重生成 |
...
文本 embedding 的复用按实际描述内容、分段与编码指纹判定；FigureBundle 换配对只更新新 manifest 的关联，不能在旧 generation 原地改 metadata。
...
每个可检索 figure 同时绑定 SVG、ChartIR、全部合格 description slices、配对验证和对应描述索引；不得让任一路单独取得普通检索可见性。
...
partial publication 可以明确隔离一个不合格 figure，不能把缺 ChartIR 的描述当作该 figure 的已完成链路发布。
...
| 读取 figure 表示与配对 | GET /v1/figures/{figure_id}/bundle → 200 | 指定 release；返回 FigureBundle、SVG/HTML representation reference、ChartIR/description references 与验证状态 |
| 提交复核决策 | POST /v1/review-decisions → 201 | evidence IDs、expected revision、批准范围/修正；产生新 correction 与后续 job |
| 搜索 | POST /v1/search → 200 | 查询、过滤条件；返回 pinned snapshot、hits 与覆盖说明；figure hit 携带 description/figure/SVG/ChartIR/release 绑定 |
| 加载 figure 上下文 | POST /v1/figure-contexts/resolve → 200 | release、description references、问题范围/预算；返回去重后的相关 ChartIR fields、全局轴/单位/图例、SVG evidence、资格与 missing fields |
...
| 解析证据 | POST /v1/evidence/resolve → 200 | snapshot 与 evidence references；figure 返回 ChartIR field → SVG element → PDF revision/page/region 链及坐标变换 |
...
figure hit 最小字段为 description_id、figure_id、source_revision_id、svg_artifact_id/digest、chart_ir_artifact_id、bundle_id、release_id、匹配文本与资格状态；可附 series/point references，但不返回拼串后的 ChartIR 充当命中文本。
上下文解析只接受该 release 内的配对：跨 release/绑定版本冲突返回 409，缺失必要 ChartIR/SVG artifact 返回 503；权限失败仍为 401/403，不改用摘要回答。
合法 ChartIR 中相关字段 unknown/unavailable 属于正常能力边界；精确问题按 answered/abstained/显式 partial 契约处理，与“必要产物丢失”分开。
...
| P5 Figure 双路与 ChartQA | 结构化 SVG/HTML、pdfspine 导出缺口、同 SVG 的 ChartIR/LLM 描述、FigureBundle、描述 embedding、命中后加载结构上下文与复核 | 原件/结构双校验；两路及索引原子发布；禁止 ChartIR/SVG embedding；错误数字/系列拒绝，缺路与跨 release 不 fallback 摘要 |
...
| A23 | 原生矢量、glyph 文字、raster figure、dash/clip 等缺失及仅 PNG 内嵌 SVG | SVG element 可回到原区域；重建与保留分级；PNG 包装不能过结构验收，未解内容不伪造 |
| A24 | ChartIR 与描述使用不同 SVG digest/figure/source revision，或只完成一路 | 配对校验失败；无独立描述发布；同源一致仍须对 PDF 原件验证 |
| A25 | 通过 recording/spy Embedding port 观察 figure 编码输入 | 只收到合格自然语言描述；ChartIR 对象、JSON/字段拼串、原始 SVG 均被拒绝且未送 provider |
| A26 | 描述数字正确但系列错误，或把估计柱高写成精确值 | 逐 claim 校验失败；不得自动以某一路覆盖另一支；不进入可发布描述索引 |
| A27 | 描述命中后 ChartIR 缺失、配对失效或引用另一 release | 明确失败；不读 latest，不把命中描述直接变成答案；正确引用链到原 PDF |
| A28 | 多段命中同 figure、相关点位预算裁剪、双轴或全局单位前提 | figure 去重；加载相关字段并保留解释所需轴/单位/图例与系列证据；预算不足不截前提算数 |
| A29 | 分别更换描述模型、ChartIR 提取器或 SVG | 描述模型不重有效 ChartIR；提取器不无端重描述/embedding但重配对；SVG 改变使双路及后代失效 |
| A30 | description index 已写入而 ChartIR/配对未通过，或发布中崩溃 | FigureBundle 不可见或完整可见；旧 release 配对不被原地修改，无仅摘要降级 |

关键不变量须全部通过，不以平均分抵消证据串版本、权限泄漏、非法拓扑或不可靠发布。
业务评测同时报告 precision、answer coverage、refusal rate、冲突识别、引用定位正确率以及各类输入的分层结果。
figure 单列 SVG 表示完整性、ChartIR 字段准确性/覆盖、描述 claim 正确率/覆盖，以及检索命中后的结构上下文正确率；不能只验描述流畅或 SVG 视觉相似。
...
不把 PNG 存档或 `<image>` 包装当作结构化 figure 完成，不把 ChartIR/原始 SVG 直接 embedding，也不以仅摘要问答替代加载 ChartIR。
...
2. **首版 ChartQA 范围**：SVG 同源双路、描述 embedding 与命中后加载 ChartIR 是本版硬要求；待冻结的是优先图种及精确读数范围。建议先支持常见柱状/折线/饼图的受限 grammar 与可验证标签，复杂双轴/瀑布等保留 unknown；视觉估计不纳入精确答案。
