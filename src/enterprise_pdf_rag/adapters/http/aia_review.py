...
def _ensure_source(snapshot: DocumentSnapshot, spec: DocumentSpec) -> None:
    ...
    if (
        ...
        or tuple(page.page_index for page in manifest.pages) != tuple(range(spec.page_count))
        ...
        raise ValueError("Persisted manifest is not the selected source; no demo fallback")
...
def create_aia_app(
    ...
    *,
    ...
    app = FastAPI(title="AIA 2026 interim source review — semantics pending", version="0.1.0.dev0")
    ...
    async def source_error(_request: Request, _error: Exception) -> JSONResponse:
        return JSONResponse(
            status_code=409,
            content={"error": "Source evidence is unavailable or inconsistent; no fallback"},
        )
    ...
    def get_manifest() -> DocumentSnapshotResponse:
        ...
        return DocumentSnapshotResponse(manifest_id=snapshot.manifest_id, manifest=manifest)
    ...
    def page_text(page_number: int) -> PageTextResponse:
        ...
        store.load(snapshot.manifest_id)
        return PageTextResponse(sidecar=read_text_sidecar(store, snapshot, page_number - 1))

    @app.get("/v1/aia/assets/{digest}")
    def asset(digest: str) -> Response:
        ref = references.get(digest)
        if ref is None:
            raise HTTPException(404, "Asset does not belong to the pinned source manifest")
        return Response(
    ...
    def source_text() -> Response:
        store.load(snapshot.manifest_id)
        return Response(content=source_text_json(store, snapshot), media_type="application/json")
    ...
    def completion(body: SourceChatRequest) -> CompletionResponse | StreamingResponse:
        if body.model != AIA_REVIEW_MODEL:
            raise HTTPException(404, "Only the selected AIA source-review model is available")
        if body.snapshot_id is not None and body.snapshot_id != snapshot.manifest_id:
            raise HTTPException(409, "Requested source snapshot differs from the pinned manifest")
        ...
        result = CompletionResponse(
            ...
            choices=(
                CompletionChoice(message=ChatMessage(role="assistant", content="\n".join(lines))),
            ),
        )
        if body.stream:
