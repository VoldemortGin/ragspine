...
def validate_processing_source(
    *,
    ...
    if (
        ...
        raise ValueError(
            "Processing source identity or page count differs from its manifest"
        )
    for record in manifest.pages:
        ...
        if (
            record.canonical.state is not StageState.SUCCEEDED
            or record.canonical.artifact is None
        ):
            raise ValueError(
                "Processing publication requires actual Canonical source observations"
            )
        ...
        if canonical != canonical_page(page):
            raise ValueError(
                "Canonical observations differ from the pinned source page"
            )
        ...
        if record.raw_partition is not None:
            ...
            if (
                record.raw_partition.state is not StageState.SUCCEEDED
                or raw_ref is None
            ):
                raise ValueError(
                    "Normalized layout requires its actual raw model layout"
                )
            ...
            if normalize_partition(page=page, partition=raw) != partition:
                raise ValueError(
                    "Normalized layout differs from its pinned raw layout and rules"
                )
        ...
        if expected != actual or len(record.objects) != len(actual):
            raise ValueError(
                "Processing objects do not cover their actual layout exactly"
            )
    if plan is not None:
