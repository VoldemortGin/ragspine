...
def append_displayed_member(
    ...
    if {
        (item.embedding_fingerprint, item.embedding_dimensions) for item in plan.members
    } != {(member.embedding_fingerprint, member.embedding_dimensions)} or (
...
def _qualified_record(
    ...
    return replace(
        record, stages=stages, qualified_claim_count=candidate.numeric_claim_count
    )
...
def create_displayed_bar_draft(
    ...
    *,
    ...
    if manifest.retrieval is None:
        raise ValueError(
            "Displayed-bar admission requires an existing retrieval snapshot"
        )
    plan, index = outputs.load_retrieval(manifest.retrieval)
    if not plan.members or {
        member.embedding_fingerprint for member in plan.members
    } != {embedder.fingerprint}:
        raise ValueError(
            "New description must use the existing embedding model fingerprint"
        )
    if any(
        (member.page_index, member.object_id) == (page_index, object_id)
        for member in plan.members
    ):
        raise ValueError(
            "Displayed-bar admission requires a previously unindexed object"
        )
    validate_processing_source(
        sources=sources, artifacts=outputs.assets, manifest=manifest, plan=plan
    )
    page = next(
        (page for page in manifest.pages if page.page_index == page_index), None
    )
    ...
    item = next(
        (item for item in partition.objects if item.object_id == object_id), None
    )
    record = next(
        (record for record in page.objects if record.object_id == object_id), None
    )
    ...
    if len(single_plan.members) != 1:
        raise ValueError(
            "Displayed-bar qualification did not produce exactly one member"
        )
    ...
    revised_plan, revised_index = append_displayed_member(
        plan, index, member, embedding
    )
    ...
    revised_page = replace(
        page,
        objects=tuple(
            qualified if obj.object_id == object_id else obj for obj in page.objects
        ),
    )
    revised = replace(
        manifest,
        pages=tuple(
            revised_page if p.page_index == page_index else p for p in manifest.pages
        ),
