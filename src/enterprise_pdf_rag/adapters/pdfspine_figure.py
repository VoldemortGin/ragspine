...
def _rgb(color: Sequence[float] | None) -> str:
    ...
    if len(values) != 3 or any(not isfinite(value) or not 0 <= value <= 1 for value in values):
...
class PdfspineFigureParser:
    ...
    def extract(self, pdf: bytes, *, page_index: int, bbox: Bounds) -> SvgArtifact:
        ...
        if any(block.get("type") != 0 for block in text.blocks):
            warnings.append("source has non-text blocks; raster coverage is not verified")
        root = ElementTree.Element(
            "svg",
            {
                "xmlns": _NAMESPACE,
                "viewBox": " ".join(
                    _number(v) for v in (bbox[0], bbox[1], bbox[2] - bbox[0], bbox[3] - bbox[1])
                ),
        ...
        for index, drawing in enumerate(drawings):
            ...
            if len(drawing.items) != 1:
                warnings.append(f"drawing {index} has a compound path; not reconstructed")
                continue
            try:
        ...
        for index, span in enumerate(_span_models(text.blocks)):
            ...
            if not _inside(span.bbox, bbox) or span.direction != (1.0, 0.0):
                warnings.append(f"text {index} is clipped or non-horizontal; not reconstructed")
                continue
            ident = f"text-{index}"
