...
class CaptureTarget(_StrictModel):
    ...
    def validate_target(self) -> "CaptureTarget":
        ...
        if (
            ...
            raise ValueError("ChartQA capture endpoint must be an explicit loopback port")
...
def capture_chart_qa(
    ...
    *,
    ...
    targets = CaptureTargets.model_validate_json(targets_payload, strict=True, extra="forbid")
    ...
    for case in gold.cases:
        ...
        if status == 200:
            body = ResponseDto.model_validate_json(response_payload, strict=True, extra="forbid")
            results.append(
                ObservedCase(
    ...
    return Observations(schema_version="chart-qa-observations-v1", results=tuple(results))


def classify_query_error(status: int, payload: bytes) -> str:
    if status in {409, 503}:
        return _ErrorBody.model_validate_json(payload, strict=True, extra="forbid").error.code
    if status == 422:
...
def write_capture_output(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            if path.read_bytes() != payload:
                raise ValueError("Capture output already exists with different bytes") from None
    finally:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    observations = capture_chart_qa(arguments.gold.read_bytes(), arguments.targets.read_bytes())
