...
def source_qualified_capture_target(
    ...
    *,
    ...
    context = StoredDisplayResolver(sources, outputs, processing_id=pin.processing_id).resolve(pin)
    ...
    member = next(member for member in plan.members if member.member_id == pin.member_id)
    ...
    evidence_methods = {period.evidence.confidence.method for period in context.point_periods}
    context_methods = {item.confidence.method for item in context.page_context}
    if any(len(methods) != 1 for methods in (period_methods, evidence_methods, context_methods)):
        raise ValueError("Capture requires one explicit confidence policy per evidence role")
