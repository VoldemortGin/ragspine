...
def process_aia_layout(
    *,
    ...
    if not 0 <= max_live_calls <= len(physical_pages):
        raise ValueError("Layout call budget cannot exceed the number of selected pages")
    ...
    pipeline = ProcessingPipeline(sources, outputs, ModelPagePartitioner(client, sources), None)
    ...
    return ProcessingRunSummary(
        ...
        layout_succeeded_pages=sum(page.partition.state == "succeeded" for page in manifest.pages),
    ...
    *,
    ...
    for context, result in zip(contexts, order, strict=True):
        ...
        try:
            ...
            allowed = False
            reason = str(error)
        directory = "object-" + sha256(context.member.object_id.encode()).hexdigest()[:20]
        decisions.append(
            RetrievalDecision(
    ...
    validation = RetrievalValidation(
        ...
        ranking_diagnostics=(
            ...
            *(
                ...
                if len(order) > 1 and len({result.relevance_score for result in order}) == 1
                else ()
            ),
        ),
