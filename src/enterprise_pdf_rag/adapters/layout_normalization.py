...
def _source_bbox(page: PageInput, item: LayoutObject) -> tuple[float, float, float, float]:
...
def normalize_partition(*, page: PageInput, partition: PagePartition) -> PagePartition:
    ...
    if partition.schema_version == NORMALIZATION_VERSION or any(
        item.model_bbox is not None or item.model_kind is not None for item in partition.objects
    ):
    ...
    for item in partition.objects:
        ...
        omission = _REVIEWED_OMISSIONS.get((page.page_index, frozenset(item.source_span_ids)))
        if (
        ...
        if candidate is not None:
            ...
            if reviewed_bbox != bbox:
                bbox = reviewed_bbox
                rules.append("reviewed-chart-candidate-outward-union-v1:" + candidate.object_id)
        ...
        if p20_sensitivity is not None:
            ...
            rules.extend(
                (
                    ...
                    "reviewed-group-candidate-outward-union-v1:" + p20_sensitivity.object_id,
        ...
        if (
            ...
            kind = ObjectKind.GROUP
            rules.append("root-source-review-kind-correction-v1:Text->Group:p018-kpi-card")
        if (
            page.source_sha256 == SOURCE_SHA256
            ...
            list_ordered = False
            rules.append("root-source-review-kind-correction-v1:Text->List:p008-check-statements")
        objects.append(
            replace(
                corrected,
                kind=kind,
    ...
    if p05_parent_id is not None:
        reviewed = Confidence(None, "root-source-render-review; uncalibrated manual candidate")
        objects.extend(
            (
                LayoutObject(
                ...
                LayoutObject(
                    ...
                    source_span_ids=(),
                    interpretation=(
                        "Observed embedded raster occurrence; content semantics remain unqualified."
                    ),
                    confidence=reviewed,
                    verification=Verification.PENDING,
                    parent_id=_P05_DIAGRAM_ID,
                    normalization=("root-source-review-added-native-raster-v1:p005-image-1",),
                ...
                LayoutObject(
                    ...
                    verification=Verification.PENDING,
                    parent_id=_P05_DIAGRAM_ID,
                    normalization=("root-source-review-added-native-raster-v1:p005-image-2",),
