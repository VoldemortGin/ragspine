...
class BarGold(StrictModel):
    ...
    def validate_gold(self) -> "BarGold":
        ...
        if len(facts) != len(self.facts) or len({c.case_id for c in self.cases}) != len(self.cases):
        ...
        for fact in self.facts:
            if (
                ...
                raise ValueError("Gold positive fact differs from its source occurrence")
        for case in self.cases:
            ...
            if (
                ...
                raise ValueError("Gold positive expected result differs from source fact")
        return self
...
class BarReleaseBindings(StrictModel):
    ...
    def validate_identities(self) -> "BarReleaseBindings":
        ...
        for value, prefix in (
            ...
            if not value.startswith(prefix + ":") or not is_digest(value[len(prefix) + 1 :]):
...
class BarCaptureTarget(StrictModel):
    ...
    def validate_pin(self) -> "BarCaptureTarget":
        if self.release is not None and self.release.pin != QueryPinDto(
            ...
            raise ValueError("Capture pin differs from independently checked publication")
        return self
