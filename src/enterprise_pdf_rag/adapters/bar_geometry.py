...
def match_visible_bar_labels(
    *,
    ...
    for vector in vectors:
        if vector.kind != "fill" or vector.alpha != 255 or not _inside(vector.bounds, region):
            continue
        try:
    ...
    geometry = match_direct_bar_labels(bars=tuple(candidates), spans=spans, region=region)
...
def _arrow(vector: BarVectorPaint, minimum_width: float) -> bool:
    ...
    return (
        ...
        and max(head_box[2] - head_box[0], head_box[3] - head_box[1]) <= minimum_width * 0.5
    ...
    if vector.kind == "fill":
        ...
        if (
            ...
            < min(
                ...
                if other.native_path_ref in {point.bar.native_path_ref for point in geometry.points}
            )
        ):
            return "background_underlay"
        if (
            _arrow(vector, minimum_width)
            and not any(vector.intersects(box) for box in (*bars, *_corridors(geometry)))
    ...
    if endpoints is not None:
        ...
        if len(crossed) == 1:
            ...
            if (
                ...
                and not any(vector.intersects(corridor) for corridor in _corridors(geometry))
            ):
...
def _column(span: TextSpan, bounds: Bounds) -> bool:
    ...
    return left + width * 0.1 < center < right - width * 0.1 and overlap >= 0.75 * (x1 - x0)
    ...
    *, bars: tuple[NativeBarPaint, ...], spans: tuple[TextSpan, ...], region: Bounds
    ...
    for bar in ordered:
        ...
        percentage_displays = tuple(
            span for span in spans if "%" in span.text and _adjacent(span, bounds, above=True)
        )
        ...
        if literal is not None and any(
            span.span_id != literal.span_id and span.text.strip() and _near_same_line(literal, span)
            for span in spans
        ):
