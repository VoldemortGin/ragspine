...
def _rectangle(commands: tuple[Command, ...], matrix: Matrix) -> Bounds:
    ...
    points = tuple(
        _transform(matrix, (value[0], value[1])) for _, value in commands[:-1]
    )
    if (
        len(set(points)) != 4
...
def _native_paths(svg: str) -> tuple[_NativePath, ...]:
    ...
    def walk(
        ...
        if tag == "path":
            source = node.get("d", "")
            bounds = _bounds(
                tuple(_transform(matrix, p) for p in _path_controls(source))
            )
            ...
            if not all(
                isfinite(value) for value in (opacity, stroke_opacity, stroke_width)
            ):
...
def _source_commands(payload: dict[str, object]) -> tuple[Command, ...]:
    ...
    def emit(kind: str, *points: Point) -> None:
        parts.append(
            kind + " ".join(str(_rounded(value)) for point in points for value in point)
        )
...
def _paint_bounds(path: _NativePath) -> Bounds:
    return (
        _expand(
            path.bounds,
            path.stroke_width
            * max(hypot(*path.matrix[:2]), hypot(*path.matrix[2:4]))
            / 2,
        )
        if path.stroke != "none"
        else path.bounds
            ...
            if event.kind == "stroke":
                bounds = _expand(
                    ...
                    TypeAdapter(float).validate_python(
                        event.payload["width"], strict=True
                    )
                    / 2,
                )
            if not _intersects(bounds, roi):
                continue
            alpha = TypeAdapter(int).validate_python(
                event.payload["alpha"], strict=True
            )
            color = f"#{TypeAdapter(int).validate_python(event.payload['color'], strict=True):06x}"
            fill_rule = (
                "evenodd"
                if event.kind == "fill"
                and TypeAdapter(bool).validate_python(
                    event.payload["even_odd"], strict=True
                )
                else "nonzero"
            )
...
def _read_source_trace(pdf: bytes, prepared: PreparedFigure) -> _SourcePaintTrace:
    ...
    with pdfspine.open(stream=pdf, filetype="pdf") as document:
        ...
        if (
            ...
            or sha256(expected_crop.encode()).hexdigest()
        ...
        rebuilt = prepare_figure(
            ...
            context_span_ids=tuple(
                context.source_span_id for context in prepared.page_context
            ),
        )
        if (
            rebuilt.svg.source,
            ...
            repr(
                tuple(
                    (
                        ...
                        event.bounds,
                        event.payload,
                        sha256(event.font_buffer).hexdigest()
                        if event.font_buffer
                        else None,
                    )
                    for event in events
                )
...
def build_source_paint_proof(
    pdf: bytes, *, prepared: PreparedFigure
) -> SourcePaintProof:
