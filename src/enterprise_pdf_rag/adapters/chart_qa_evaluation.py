...
class ChartQaGold(_StrictModel):
    ...
    def validate_fixture(self) -> "ChartQaGold":
        ...
        for case in self.cases:
            if case.case_class != "positive" and case.expected.business_status == "answered":
...
def evaluate_chart_qa(gold_payload: bytes, observations_payload: bytes) -> EvaluationReport:
    ...
    if duplicate_ids:
        global_diagnostics.append("duplicate_case_ids:" + ",".join(sorted(duplicate_ids)))
    if unexpected_ids:
        global_diagnostics.append("unexpected_case_ids:" + ",".join(sorted(unexpected_ids)))
    ...
    metrics = EvaluationMetrics(
        ...
        unanswerable_refusal_recall=_ratio(unanswerable_refused, unanswerable_business_total),
        answerable_over_refusal_rate=_ratio_required(positive_abstained, positive_total),
        ...
        hard_negative_escape_rate=_ratio_required(hard_negative_answered, hard_negative_total),
        ...
        if status == "answered":
            ...
            if answer is None:
                ...
                for field_name in ("value", "unit", "value_kind", "raw_display"):
                    if getattr(answer, field_name) != getattr(case.expected, field_name):
            ...
            correct_citation_count, returned_citation_count, input_errors = citation_result
...
def _citation_matches(gold: ChartQaGold, citation: CitationDto, expected: GoldOccurrence) -> bool:
    if (
...
def _evaluate_calculation(gold: ChartQaGold, case: GoldCase, body: ResponseDto) -> tuple[str, ...]:
    ...
    expected_field_paths = tuple(f"points.{point_id}.value" for point_id in expected.fact_ids)
    ...
    for claim in body.inputs:
        value_citations = tuple(
            citation for citation in claim.citations if citation.field_path.endswith(".value")
        )
    ...
    qualification_ids = {
        citation.qualification_id for claim in body.inputs for citation in claim.citations
    }
    if (
    ...
    if (
        tuple(item.value for item in receipt.inputs) != tuple(claim.value for claim in body.inputs)
...
def _validate_positive_arithmetic(case: GoldCase, facts: tuple[GoldFact, ...]) -> None:
    ...
    expected = case.expected
    if case.request.operation == "lookup":
        if len(expected.fact_ids) != 1 or _decimal(expected.value) != values[expected.fact_ids[0]]:
...
def _request_is_true(request: GoldRequest, scope: GoldScope, facts: tuple[GoldFact, ...]) -> bool:
...
def write_evaluation_bundle(*, gold_path: Path, observations_path: Path, output_root: Path) -> Path:
...
def read_evaluation(folder: Path) -> EvaluationReport:
    ...
    report = CHART_QA_REPORT_ADAPTER.validate_json(report_payload, strict=True, extra="forbid")
    if (
...
def _write_immutable(path: Path, payload: bytes) -> None:
    ...
    try:
        ...
        try:
            ...
            if path.read_bytes() != payload:
                raise ValueError(f"Evaluation artifact already differs: {path.name}") from None
