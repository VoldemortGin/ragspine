...
def _source_elements(
    ...
    category = next(element for element in full if element.source_span_id == point.category.span_id)
    literal = next(element for element in full if element.source_span_id == point.literal.span_id)
    fragments = tuple(
        element for element in svg.elements if element.source_span_id == literal.source_span_id
    )
    number = next((element for element in fragments if element.text == literal.text[:-1]), None)
...
def qualify_displayed_bar(
    ...
    *,
    ...
    geometry = match_visible_bar_labels(
        ...
        spans=tuple(span for span in prepared.paint_text_spans if span.span_id in local_ids),
    ...
    titles = tuple(
        ...
        and element.anchor.bbox[3] < min(point.bar.bbox[1] for point in geometry.geometry.points)
    )
    if len(titles) != 1:
    ...
    for index, claim in enumerate(description.claims):
        ...
        if (
            ...
            or claim.text != f"{claim.series} for {claim.category}: {claim.value} {claim.unit}."
        ):
            raise ValueError("description_contains_unqualified_bar_semantics")
        expected = (title, category, number, unit)
        if set(claim.evidence.element_ids) != {element.element_id for element in expected}:
    ...
    if len(covered) != len(points) or set(covered) != {point.category.text for point in points}:
    ...
    for point in points:
        for name, evidence in (
            ...
            fields.append(FieldOccurrence(f"points.{point.point_id}.{name}", evidence.element_ids))
    role_id = content_id(
        "bar-paint-roles-v1",
        (
            geometry.roles,
