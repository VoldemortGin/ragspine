...
def _summary(manifest: ProcessingManifest) -> str:
    ...
    qualified = sum(item.qualified_claim_count for page in manifest.pages for item in page.objects)
...
def _chart_qa_records(run: Path, manifest: ProcessingManifest) -> str:
    ...
    for folder in sorted((run / "chart-qa-v2-evaluations").glob("*")):
        ...
        if manifest.retrieval is None:
            raise ValueError("Displayed lookup evaluation requires a pinned retrieval release")
...
def _coverage(outputs: ProcessingStore, manifest: ProcessingManifest) -> tuple[str, str]:
    ...
    table += (
        "".join(
            "<tr>" + "".join(f"<td>{escape(str(row[key]))}</td>" for key in columns) + "</tr>"
            for row in rows
        )
        + "</table>"
    )
