...
def _positive(
    ...
    if len(body.inputs) == 1:
        ...
        if len(by_role) == len(claim.citations):
            citations = sum(
                role in by_role
                and _citation_correct(gold, release, fact, role, by_role[role])
        ...
        period_ok = (
            ...
            and _unknown(
                period.evidence.confidence, release.period_evidence_confidence_method
            )
        )
    else:
...
def _evaluate_case(
    ...
    if json.loads(observed.request_json) != json.loads(query_payload(case, target)):
        return failure.model_copy(
            update={"diagnostics": ("captured_request_mismatch",)}
        )
    ...
    if observed.http_status != 200:
        reason = classify_query_error(
            observed.http_status, observed.response_json.encode()
        )
        passed = (
    ...
    body = BarResponseDto.model_validate_json(
        observed.response_json, strict=True, extra="forbid"
    )
    if (
...
def validate_bar_targets(gold: BarGold, targets: BarCaptureTargets) -> None:
    ...
    for case in gold.cases:
        ...
        if release is None or (
            ...
            or release.normalization.raw_response_sha256
            != gold.normalization.raw_response_sha256
...
def evaluate_bar_chart_qa(
        ...
        if observed is not None:
            try:
                result = _evaluate_case(
                    gold, case, targets.targets[case.request.target], observed
                )
    ...
    metrics = BarEvaluationMetrics(
        ...
        citation_exactness=_ratio(
            citations, sum(result.returned_citations for result in results)
        ),
        ...
        hard_negative_escape_rate=_ratio(
            negative_answers, len(gold.cases) - len(positives)
        ),
    )
        ...
        strata=BarEvaluationStrata(
            business_refusals=_stratum(
                tuple(
                    result
                    for result in results
                    if result.expected_status == "abstained"
                    and result.expected_http_status == 200
                )
            ),
            unsupported_requests=_stratum(
                tuple(
                    result for result in results if result.expected_http_status == 422
                )
            ),
            source_and_pin_faults=_stratum(
                tuple(
                    result
                    for result in results
                    if result.expected_http_status in (409, 503)
                )
            ),
        ),
        cases=tuple(results),
...
def read_bar_evaluation(
    ...
    report = BarEvaluationReport.model_validate_json(
    ...
    if (processing_id is None) != (snapshot_id is None):
        raise ValueError(
            "Evaluation release requires both processing and snapshot pins"
        )
    if processing_id is not None:
        gold = load_bar_gold((folder / "gold.json").read_bytes())
        targets = BarCaptureTargets.model_validate_json(
            (folder / "targets.json").read_bytes()
        )
        validate_bar_targets(gold, targets)
        for case in gold.cases:
            if case.case_class != "positive":
            ...
            if (target.processing_id, target.snapshot_id) != (
                ...
                raise ValueError(
                    "Evaluation positive target belongs to another release"
                )
    return report
