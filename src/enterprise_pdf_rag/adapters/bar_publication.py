...
def _inputs(
    ...
    *,
    ...
    if prepared.model_view != view or prepared.svg.svg.encode() != assets.get(
        source_svg
    ):
        raise ValueError("displayed_bar_view_or_svg_source_mismatch")
    chart = TypeAdapter(ChartIR).validate_json(
        assets.get(raw_chart), strict=True, extra="forbid"
    )
...
def _context(inputs: _Inputs) -> tuple[PageContextCitation, SourcePaintBarProof]:
    spans = tuple(
        span for span in inputs.page.text.spans if span.text == ACTUAL_FX_CONTEXT
    )
    if len(spans) != 1:
    ...
    if any(
        element.source_span_id == span.span_id
        for element in inputs.prepared.svg.elements
    ) or (
...
def build_displayed_bar_candidate(
    ...
    *,
    ...
    receipt = BarPublicationReceipt(
        ...
        page_context_proof=put(
            TypeAdapter(SourcePaintBarProof).dump_json(context_proof)
        ),
...
def resolve_displayed_bar_member(
    ...
    if (
        normalized != inputs.normalized.description
        or normalization != inputs.normalized.receipt
    ):
    ...
    proof = verify_bar_source_paint_proof(
        inputs.pdf, prepared=inputs.prepared, proof=recorded
    )
    projection = _qualify(inputs, proof)
    context, context_proof = _context(inputs)
    ...
    chart = TypeAdapter(ChartIR).validate_json(
        assets.get(member.ir), strict=True, extra="forbid"
    )
