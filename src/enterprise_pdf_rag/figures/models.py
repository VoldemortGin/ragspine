    ...
    def __post_init__(self) -> None:
        if self.evidence_kind is EvidenceKind.SOURCE_TEXT_OBSERVATION:
            if (
                ...
                raise ValueError("Source text observations require Unicode [start,end) offsets")
        elif self.source_span_id is not None or self.text_range is not None:
            raise ValueError("Native SVG elements must not imply an unverified text-span mapping")
