...
from enterprise_pdf_rag.figures.chart_qa.displayed_ports import DisplayedContextResolver
from enterprise_pdf_rag.figures.chart_qa.evidence import citation, source_display
...
class DisplayedChartQAService:
    ...
    def _answer(
        self, question: ChartQuestion, context: DisplayedLookupContext
    ) -> DisplayedAnswer:
        ...
        point = next(
            (p for p in context.chart.points if p.point_id == selector.point_id), None
        )
        if point is None:
            return abstain(question, DisplayedRefusalReason.INSUFFICIENT_EVIDENCE)
        period = next(
            p for p in context.point_periods if p.point_id == selector.point_id
        )
