"""领域切块预设（Item ⑩）：三个【薄】LayoutAwareChunker 子类，仅覆写标题谓词 _is_heading。

设计取向：不新增引擎、零三方依赖、确定性。每个预设只调优「什么算小节边界」这一件事，
其余（小节内复用 chunk_document 预算贪心、parent_id/heading 标注、locator/全局段号、参数校验、
空文本处理）全部从 LayoutAwareChunker 继承。经 make_chunker 选用，默认切块路径不受影响。

三个预设：
    - LawsChunker（laws/law/legal）：条款层级——每个「第N条」自成小节，条级子块。
    - BookChunker（book/chapter）：章节层级——「第N章/节/篇/部/回/讲」起小节，正文散文不误判。
    - QaChunker（qa/faq）：问答对成对——每个问句起小节，其后答案段落随之同块共享 parent_id。
"""
