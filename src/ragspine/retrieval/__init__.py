...
Submodules:
    ...
    page_images/ — opt-in 图文混合上下文（RAGSPINE_PAGE_IMAGES=off|tagged|all，on=all）：页图映射表 + 前 N 页附页图引用，tagged 按页标签只留一部分（只删不增）；含 RESTRICTED 块的页不发图；默认 off、字节不变。
    page_parent/ — 页级父子 + 按页去重（RAGSPINE_PAGE_PARENT=off|dedup|page+child）：同页只返回一次、代表块带整页上下文；服务 / facade 默认 page+child，off 字节不变。
