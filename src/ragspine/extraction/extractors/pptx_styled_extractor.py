...
旧 src/ragspine/extraction/extractors/pptx_extractor.py 是 MVP 确定性抽取器（表格 + 原生图表 -> Fact），
...
class NoteFragment:
    ...
    字段语义约定：
        ...
        glossary_hits:  经 src/ragspine/common/glossary.py 命中的指标代码列表（确定性词典匹配，
...
def extract_note_fragments(path: str | Path) -> list[NoteFragment]:
    ...
    确定性规则（零 LLM）：
        ...
        - text 空白归一化。
        - glossary_hits = 经 src/ragspine/common/glossary.py 确定性词典命中的指标代码列表
