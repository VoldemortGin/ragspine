...
定位与命名约定（与 pptx_styled 对齐，保证两条 pptx 路 IR 同形、下游零分支）：
    ...
    - resolved_rgb 一律 None —— 富填充/颜色语义留 W3d（与 docspine / pdf_spine 同口径；
      python-pptx 的 theme 色解析仍由 pptx_styled 默认路径承担）。
...
def extract_grids(path: str | Path) -> list[StyledGrid]:
    ...
    文本、resolved_rgb 恒 None；血缘 source_doc_id / source_file_hash 照常。无表格 -> []。
