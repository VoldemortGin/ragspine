...
from ragspine.extraction.evidence.figures.chart_qa.evidence import check_context
...
def _page_context(context: DisplayedLookupContext) -> None:
    ...
    if (
        ...
        or any(item.source_span_id == note.source_span_id for item in context.svg.elements)
        or (
            max(note.source.bbox[0], source.bbox[0]) < min(note.source.bbox[2], source.bbox[2])
            and max(note.source.bbox[1], source.bbox[1]) < min(note.source.bbox[3], source.bbox[3])
        )
    ):
        raise _invalid("Page context is unqualified, changed, or confused with crop evidence")
...
def check_displayed_evidence(context: DisplayedLookupContext) -> None:
    ...
    if (
        ...
        or re.fullmatch(r"description-normalization-v1:[0-9a-f]{64}", audit.receipt_id) is None
        ...
        or audit.original_typed_description_artifact_id == audit.normalized_description_artifact_id
    ):
        raise _invalid("Normalization audit has an unknown rule or incompatible branch identity")
    ...
    if (
        ...
        or not set(chart.title.evidence.element_ids).issubset(raw.title.evidence.element_ids)
        ...
        raise _invalid("Displayed projection changed its raw binding or global chart semantics")
    ...
    if set(fields) != set(proved) or any(
        ...
        raise _invalid("Receipt does not bind every displayed field to its exact occurrence")
    if len(context.point_periods) != len(chart.points) or {
        ...
        raise _invalid("Every qualified point requires exactly one period-role interpretation")
    for role in context.point_periods:
        ...
        if (
            ...
            raise _invalid("Period role is not the exact qualified raw category occurrence")
