...
from ragspine.extraction.evidence.figures.ports import (
    ChartExtractor,
...
from ragspine.extraction.evidence.figures.validation import validate_pair, validate_svg
...
class FiguresService:
    ...
    def build(self, svg: SvgArtifact, *, snapshot_id: str) -> FigureBundle:
        ...
        if chart is None:
            raise FigureError(FailureCode.MISSING_CHART, "ChartIR branch produced no artifact")
    ...
    def pair(
        ...
        *,
        ...
        key = content_id("embedding-v1", (description.digest, self.embedder.fingerprint))
        ...
        bundle = FigureBundle(
            ...
            key,
            qualified.receipt.artifact_id if self.mode is ExecutionMode.PRODUCTION else None,
        )
    ...
    def qualify_pair(
        ...
        if chart.execution_mode is not self.mode or description.execution_mode is not self.mode:
            raise FigureError(
                FailureCode.EXECUTION_MODE,
        ...
        if qualified.chart.binding != svg.binding or qualified.description.binding != svg.binding:
            raise FigureError(
                FailureCode.BINDING_MISMATCH,
                "Qualified projections changed source bindings",
            )
        self._check_qualification(svg, qualified.chart, qualified.description, qualified.receipt)
    ...
    def _check_qualification(
        ...
        expected = {field.field_path: field.element_ids for field in qualification.fields}
        if (
            len(actual_fields) != len(expected)
            or len({path for path, _ in actual_fields}) != len(actual_fields)
            or any(expected.get(path) != evidence.element_ids for path, evidence in actual_fields)
        ):
            raise FigureError(
                FailureCode.INVALID_EVIDENCE,
    ...
    def search(self, query: str, *, snapshot_id: str, limit: int = 5) -> tuple[FigureHit, ...]:
    ...
    def resolve(self, hit: FigureHit, *, snapshot_id: str) -> ReasoningView:
        ...
        if bundle is None:
            raise FigureError(FailureCode.MISSING_ARTIFACT, "No figure bundle in this snapshot")
