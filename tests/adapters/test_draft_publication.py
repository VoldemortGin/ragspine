...
def test_cli_qualify_emits_expected_json(
...
def test_index_fails_closed_on_tampered_evidence(tmp_path: Path) -> None:
    ...
    description_ref = outputs.assets.put(
        TypeAdapter(ObjectDescription).dump_json(
            replace(description, producer="forged-producer")
        ),
        media_type="application/json",
    )
...
def test_cli_publish_emits_expected_json(
    tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    sources, outputs, indexed_id = _indexed_draft(tmp_path)
    code = main(
...
def test_cli_publish_no_activate_source(
    tmp_path: Path, capsys: CaptureFixture[str]
) -> None:
    sources, outputs, indexed_id = _indexed_draft(tmp_path)
    (sources.root / "current-manifest").unlink()
...
def test_relative_stores_resolve_to_absolute_paths(
    ...
    assert review == (
        (tmp_path / "processed").resolve()
        / "runs"
        / indexed.indexed_processing_id
        / "review.html"
    )
