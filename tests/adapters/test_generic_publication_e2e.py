...
from tests.adapters.test_pdf_ingestion import authored_pdf
...
def test_generic_pdf_ingest_qualify_index_publish_retrieve_offline(
    ...
    assert (
        len(calls) == 3
    )  # one layout call per page; Text needs no semantic model call
    ...
    assert (
        sources.root / "current-manifest"
    ).read_text().strip() == ingest.source_manifest_id
