...
def test_configured_app_search_uses_only_query_embedding_and_preserves_guard(
    ...
    monkeypatch.setattr(
        app_module, "create_aia_app", partial(create_aia_app, spec=spec)
    )
    monkeypatch.setattr(
        "enterprise_pdf_rag.adapters.local_models._send_local_once", sender
    )
        ...
        async def exercise() -> None:
            async with AsyncClient(
                ...
                if configuration != "valid":
                    expected = (
                        409
                        if configuration in {"wrong-model", "wrong-dimensions"}
                        else 503
                    )
                ...
                assert (
                    "test-embedding-secret"
                    not in response.text + context.text + refused.text
                )
                assert (
                    "unrelated-llm-secret"
                    not in response.text + context.text + refused.text
                )
