...
def test_unfaithful_narrative_answer_fails_faithfulness_gate():
    ...
    bad.narrative_answer = f"{_HK_REG_CTX}\n香港 REVENUE 暴增到 9999 亿美元，利润翻倍。"
...
def test_faithfulness_catches_fabrication_against_real_retrieved_context(tmp_path):
    ...
    try:
        ...
        ok_outcome = CaseOutcome(case_id="n1", narrative_answer=grounded, retrieved_context=context)
...
def test_answer_accuracy_gate_passes_for_on_topic_and_fails_offtopic():
    ...
    case = _narrative_case()
    assert (
        evaluate([case], {"n1": on_topic}, reference_texts=refs).metrics[ANSWER_ACCURACY].passed
        == 1
    )
    assert (
        evaluate([case], {"n1": off_topic}, reference_texts=refs).metrics[ANSWER_ACCURACY].passed
        == 0
    )
...
def test_eval_narrative_reference_texts_covers_expected_docs():
    ...
    for doc in ("REG_WATCH_HK.pptx", "HK_QBR_2025Q4.pptx", "CN_QBR_2025.pptx", "REG_WATCH_CN.pptx"):
