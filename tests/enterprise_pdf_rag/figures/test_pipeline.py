...
def evidence(*ids: str) -> Evidence:
    return Evidence(ids, Verification.VERIFIED, Confidence(None, "fixture exact labels"))
...
def test_numeric_claim_binds_period_and_rejects_extra_conclusions() -> None:
    ...
    chart = replace(Extractor().extract(svg), period=TextField("1H26", evidence("period")))
    ...
    for changed in (
        ...
        replace(claim, text=claim.text + " Agency is market-leading due to policy changes."),
    ):
        ...
        return ChartIR(
            ...
            (
                ChartPoint(
                    ...
                    TextField("USD million", evidence("unit")),
                    NumericObservation(Decimal("42"), ValueKind.EXPLICIT, evidence("value")),
                ),
            ),
            "fixture-chart@1",
            Verification.VERIFIED,
...
@pytest.mark.parametrize("field", ["figure_id", "source_revision", "svg_artifact_id", "svg_digest"])
def test_pair_rejects_wrong_svg_binding_before_embedding(branch: str, field: str) -> None:
...
def test_unqualified_inputs_never_reach_embedding(part: str) -> None:
    ...
    if part == "svg":
        ...
        point = replace(chart.points[0], value=replace(chart.points[0].value, evidence=rejected))
...
def test_description_claim_must_match_source_fields_before_embedding(
    ...
    if error_kind == "number":
        claim = replace(claim, value=Decimal("420"), text="Revenue for 2024: 420 USD million.")
    elif error_kind == "series":
        claim = replace(claim, series="Profit", text="Profit for 2024: 42 USD million.")
    elif error_kind == "unit":
        claim = replace(claim, unit="EUR million", text="Revenue for 2024: 42 EUR million.")
        ...
        claim = replace(claim, text='{"Revenue": "2024", "value": 42, "unit": "USD million"}')
    elif error_kind == "negated":
        claim = replace(claim, text="Revenue for 2024: not 42 USD million.")
    elif error_kind == "unsupported_trend":
        claim = replace(claim, text="Revenue for 2024: 42 USD million, a rapid decline.")
    else:
...
def test_svg_mapping_cannot_disagree_with_xml_or_pdf_source(corruption: str) -> None:
    ...
    if corruption == "xml_text":
        ...
        svg = replace(svg, elements=(*svg.elements[:-1], replace(svg.elements[-1], anchor=source)))
    else:
...
def test_context_never_falls_back_to_summary_when_chart_is_missing_or_replaced(
    ...
    if corruption == "replaced_chart":
        repository.charts[bundle.chart_ir_artifact_id] = replace(chart, producer="other@2")

    with pytest.raises(FigureError):
        service.resolve(hit, snapshot_id="release-1")


@pytest.mark.parametrize("kind", [ValueKind.ESTIMATED, ValueKind.UNAVAILABLE, ValueKind.DERIVED])
...
def test_same_number_in_different_series_still_requires_matched_point_evidence() -> None:
    svg = figure()
    ...
    profit = replace(
        ...
        value=NumericObservation(Decimal("42"), ValueKind.EXPLICIT, evidence("profit-value")),
    )
    chart = replace(chart, points=(*chart.points, profit))
    description = Describer().generate(svg)
    wrong = replace(description.claims[0], text="Profit for 2024: 42 USD million.", series="Profit")
    embedder = Embedder()
    service = FiguresService(
        Extractor(),
        Describer(),
    ...
    with pytest.raises(FigureError) as error:
        service.pair(svg, chart, replace(description, claims=(wrong,)), snapshot_id="release-1")
...
def test_same_text_at_another_occurrence_cannot_replace_series_evidence() -> None:
    ...
    chart = replace(
        chart,
        points=(replace(chart.points[0], series=TextField("Revenue", evidence("decoy-series"))),),
    )
    description = Describer().generate(svg)
    description = replace(
        description,
...
def test_self_verified_svg_requires_independent_exact_source_receipt(
    ...
    if corruption == "missing":
        ...
        receipt = replace(original, source=replace(original.source, document_sha256="b" * 64))
    elif corruption == "bbox":
        receipt = replace(original, source=replace(original.source, bbox=(0.0, 0.0, 100.0, 50.0)))
    else:
        receipt = replace(original, binding=replace(original.binding, svg_digest="b" * 64))
