...
def test_missing_model_occurrences_are_retained_with_individual_diagnostics(
    ...
    assert reconciled.objects == raw.objects  # Proximity does not create a relationship.
    assert reconciled.unassigned_span_ids == tuple(f"s{n}" for n in range(1, omitted + 1))
    assert len(reconciled.diagnostics) == omitted
    assert raw.unassigned_span_ids == () and item.verification == "pending"
    with pytest.raises(ValueError, match="unknown"):
        preserve_omitted_occurrences(page, replace(raw, unassigned_span_ids=("invented",)))
