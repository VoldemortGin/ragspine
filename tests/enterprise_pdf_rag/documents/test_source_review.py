...
def test_source_model_names_file_and_returns_real_observation(
    ...
    async def exercise() -> None:
        async with AsyncClient(
            ...
            assert [item["id"] for item in models] == ["aia-2026-interim-source-review-v1"]
            ...
            text = "".join(packet["choices"][0]["delta"].get("content", "") for packet in packets)
            ...
            asset = await client.get("/v1/aia/assets/" + snapshot["manifest"]["source"]["sha256"])
...
def test_wrong_page_region_sidecar_is_rejected_before_review(
    ...
    wrong_ref = store.put(json.dumps(asdict(wrong_sidecar)).encode(), media_type="application/json")
    ...
    async def exercise() -> None:
        async with AsyncClient(
            ...
            for patch, status in (
                ...
                response = await client.post("/v1/chat/completions", json={**base, **patch})
...
def test_default_chat_shows_real_processing_status_and_links_not_old_focus_page(
    ...
    async def exercise() -> None:
        async with AsyncClient(
            transport=ASGITransport(app=create_aia_app(store, spec=spec, processing=outputs)),
            base_url="http://test",
        ) as client:
            response = await client.post(
                "/v1/chat/completions",
