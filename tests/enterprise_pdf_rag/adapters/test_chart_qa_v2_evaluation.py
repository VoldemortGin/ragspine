...
def test_negative_strata_keep_independent_denominators_and_exact_http_outcomes(
    ...
    assert (
        sum(getattr(report.strata, name).unexpected_answers for name in strata if name != stratum)
        == 0
    )
    assert all(
...
def test_response_mutations_cannot_pass(path: tuple[str | int, ...], value: JsonValue) -> None:
...
def test_all_abstain_and_false_answer_to_missing_value_fail() -> None:
    ...
    wrong = observations.model_copy(
        update={"results": (*observations.results[:2], escaped, *observations.results[3:])}
    )
    report = evaluate_bar_chart_qa(
...
def test_targets_are_independent_and_observations_are_hash_bound() -> None:
    ...
    with pytest.raises(ValueError, match="publication"):
        evaluate_bar_chart_qa(gold_bytes, changed_targets, observations.model_dump_json().encode())
...
def test_four_file_bundle_keeps_targets_and_http_bytes_immutable(
    ...
    folder = write_bar_evaluation_bundle(tmp_path, gold_bytes, target_bytes, observed_bytes)
    ...
    with pytest.raises(ValueError, match="release"):
        read_bar_evaluation(folder, processing_id="f" * 64, snapshot_id=positive_target.snapshot_id)
    assert write_bar_evaluation_bundle(tmp_path, gold_bytes, target_bytes, observed_bytes) == folder
