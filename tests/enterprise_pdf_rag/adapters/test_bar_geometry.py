...
@pytest.mark.parametrize("role", ["white_cover", "thick_break", "unexpected_line", "clipped_bar"])
...
def test_direct_labels_have_unique_bars_and_unlabelled_height_stays_unavailable() -> None:
    result = match_direct_bar_labels(bars=bars(), spans=spans(), region=(0.0, 0.0, 140.0, 130.0))
...
def test_unsupported_full_display_cannot_be_reduced_to_an_exact_number(
    ...
    with pytest.raises(ValueError, match="unsupported_bar_percent_display"):
        match_direct_bar_labels(bars=bars(), spans=observed, region=(0.0, 0.0, 140.0, 130.0))
...
def test_floating_rectangles_do_not_form_a_common_baseline_bar_family() -> None:
    ...
    changed = replace(
        ...
        commands=tuple(
            (
                operation,
                tuple(value - 5.0 if index % 2 else value for index, value in enumerate(values)),
            )
