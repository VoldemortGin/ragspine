...
def targets_fixture(gold: BarGold) -> BarCaptureTargets:
    ...
    release = BarReleaseBindings(
        pin=QueryPinDto(processing_id="1" * 64, snapshot_id="2" * 64, member_id="3" * 64),
...
def response_fixture(gold: BarGold, case: GoldCase, target: BarCaptureTarget) -> BarResponseDto:
    body = BarResponseDto(
        schema_version="chart-qa-v2",
    ...
    fact = next(fact for fact in gold.facts if fact.point_id == case.expected.fact_ids[0])
    confidence = ConfidenceDto(score=None, method="source-qualified explicit occurrence")
    ...
    return body.model_copy(
        update={
            ...
            "answer": AnswerDto(
                ...
                raw_display=fact.raw_display,
                verification="verified",
                confidence=ConfidenceDto(score=None, method=case.expected.confidence_method or ""),
            ),
            "inputs": (claim,),
            "page_context": (
                ContextDto(
