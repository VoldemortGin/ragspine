...
def test_p18_locked_region_has_stable_id_and_excludes_clipped_neighbor() -> None:
    entries = _entries(17)
    locked = next(item for item in entries if item["object_id"] == "aia-p018-distribution-mix-v1")
...
def test_p20_reviewed_group_keeps_table_alternative_and_context_out_of_crop() -> None:
    entry = next(
        item for item in _entries(19) if item["object_id"] == "aia-p020-sensitivity-scenarios-v1"
    )
...
def test_catalog_is_provisional_and_all_actual_occurrences_resolve_when_available() -> None:
    payload = _catalog()
