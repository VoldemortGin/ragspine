...
def _text(snapshot: DocumentSnapshot, page_index: int, value: str = "source words") -> TextSidecar:
    return TextSidecar(
        "source-text-v1",
...
def test_index_links_all_pages_without_embedding_all_page_svgs() -> None:
    snapshot = _snapshot()
    html = render_index(snapshot=snapshot, region_svg=SVG, region_text=_text(snapshot, 24))
...
def test_page_keeps_its_svg_text_identity_and_adjacent_navigation() -> None:
    snapshot = _snapshot()
    html = render_page(snapshot=snapshot, page_index=24, svg=SVG, text=_text(snapshot, 24, "49.00"))
    links = _Links(html).hrefs
...
def test_source_text_and_filename_are_html_escaped() -> None:
    ...
    snapshot = replace(
        snapshot,
        manifest=replace(snapshot.manifest, filename='<script>alert("filename")</script>.pdf'),
    )
    html = render_page(
        snapshot=snapshot,
        page_index=0,
...
def test_page_rejects_mislabeled_source_text(wrong_source: bool) -> None:
    ...
    text = replace(text, source_sha256="f" * 64) if wrong_source else replace(text, page_index=23)
