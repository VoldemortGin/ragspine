...
def test_explicit_chart_correction_uses_the_same_view_and_never_repairs_model_output(
    ...
    def sender(url: str, *, api_key: str, payload: bytes, timeout: float) -> bytes:
        ...
        digest = prepared.svg.digest if len(payloads) > 1 and model_corrects_digest else "0" * 63
    ...
    correction = ModelChartExtractor(client, prepared, correction_of=old.value.request_fingerprint)
    ...
    ordinary_field = first["response_format"]["json_schema"]["schema"]["properties"]["svg_digest"]
    corrected_field = second["response_format"]["json_schema"]["schema"]["properties"]["svg_digest"]
