...
def authored_font() -> bytes:
    ...
    return Path(__file__).parents[1].joinpath("fixtures/authored-label-font.ttf").read_bytes()
    ...
    with pdfspine.open() as document:
        ...
        if clipped:
            document.update_stream(
                contents[0],
                b"q\n20 50 2 20 re W n\n" + document.xref_stream(contents[0]) + b"\nQ\n",
            )
        if invisible:
            document.update_stream(
                contents[0],
...
def test_pdf_clip_cannot_turn_a_partly_visible_numeric_label_into_exact_evidence() -> None:
...
def test_serialized_proof_is_rebuilt_not_trusted_as_an_approval() -> None:
    source, prepared, _ = glyph_source()
    proof = build_source_paint_proof(source, prepared=prepared)
    changed = replace(proof, glyphs=(replace(proof.glyphs[0], character="9"), *proof.glyphs[1:]))
    ...
    *, fill_rule: str = "f*", number: str = "72%"
    ...
    def ev(evidence: Evidence) -> Evidence:
        ...
        for identifier in evidence.element_ids:
            ...
            old_span = next(
                span for span in old.paint_text_spans if span.span_id == old_element.source_span_id
            )
            target_span = next(
                span
    ...
    chart = replace(
        ...
        title=replace(chart.title, evidence=ev(chart.title.evidence)) if chart.title else None,
        period=replace(chart.period, evidence=ev(chart.period.evidence)) if chart.period else None,
        marks=(),
    )
    description = replace(
        description,
...
def test_authored_real_pdf_donut_has_a_complete_source_proof_and_numeric_positive() -> None:
    ...
    assert {point.category.text: str(point.value.value) for point in pair.chart.points} == {
        "Agency": "72",
        "Partnerships": "28",
    }
...
def test_real_p18_glyph_mapping_uses_source_font_program_not_a_paint_whitelist() -> None:
    ...
    by_span = {
        span.span_id: "".join(
            glyph.character for glyph in proof.glyphs if glyph.source_span_id == span.span_id
        )
    ...
    } == {label: label for label in ("72%", "28%", "Agency", "Partnerships", "VONB", "1H26")}
    ...
    if not object_dir.is_dir():
        pytest.skip("Pinned optional raw p18 branches absent; source glyph proof passed")
    chart = TypeAdapter(ChartIR).validate_json((object_dir / "ir.json").read_bytes(), strict=True)
