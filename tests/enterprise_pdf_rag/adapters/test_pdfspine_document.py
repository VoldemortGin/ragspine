...
def test_real_aia_pages_preserve_native_svg_and_coordinate_text_sidecars(
    ...
    assert all(
        (page.width, page.height, page.rotation) == (960.0, 540.0, 0) for page in result.pages
    )
    document = pdfspine.open(stream=source_pdf, filetype="pdf")
    try:
        assert result.pages[9].native_svg == document.load_page(9).get_svg_image(text_as_path=False)
    finally:
        document.close()
    ...
    region = PdfspineDocumentAdapter().extract_region(source_pdf, page_index=24, bbox=DIVIDEND_BBOX)
...
def test_real_dividend_region_keeps_page_coordinates_and_only_intersecting_text(
    ...
    result = PdfspineDocumentAdapter().extract_region(source_pdf, page_index=24, bbox=DIVIDEND_BBOX)
...
def test_rotated_actual_page_is_explicitly_unsupported(source_pdf: bytes) -> None:
    ...
    with pytest.raises(ValueError, match="Page index 24 extraction failed: Rotated pages"):
        PdfspineDocumentAdapter().extract_region(rotated_source, page_index=24, bbox=DIVIDEND_BBOX)


@pytest.mark.parametrize("page_index", [-1, 71])
def test_region_rejects_page_outside_actual_document(page_index: int, source_pdf: bytes) -> None:
