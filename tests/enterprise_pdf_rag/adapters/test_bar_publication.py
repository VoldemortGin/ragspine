...
def test_reidentified_tampering_still_fails_source_requalification(
    ...
    if mutation in {"source_paint", "footer_paint"}:
        field = "source_paint_proof" if mutation == "source_paint" else "page_context_proof"
        old = getattr(receipt, field)
        proof = TypeAdapter(SourcePaintBarProof).validate_json(assets.get(old), strict=True)
        changed = replace(
        ...
        member = replace(
            member,
            lineage_refs=tuple(new if ref == old else ref for ref in member.lineage_refs),
        )
    ...
    member = replace(
        member,
        qualification=assets.put(receipt.model_dump_json().encode(), media_type="application/json"),
    )
    with pytest.raises(ValueError, match=r"revalidation_mismatch|lineage_required|source_mismatch"):
