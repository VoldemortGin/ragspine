...
def published_bar_input(
    ...
    footer_spans = tuple(
        span for span in source.page.text.spans if span.span_id not in item.source_span_ids
    )
    ...
    footer_record = ProcessingObjectAdapter(sources, outputs).process(source.page, footer)
    ...
    manifest = ProcessingManifest(
        ...
        (
            PageProcessingRecord(
                0,
                StageOutcome("canonical", "a" * 64, StageState.SUCCEEDED, "test", canonical),
                StageOutcome("partition", "b" * 64, StageState.SUCCEEDED, "test", partition),
