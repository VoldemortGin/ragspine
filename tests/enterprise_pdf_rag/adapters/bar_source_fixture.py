...
def bar_source(*, hidden_footer: bool = False, cover_footer: bool = False) -> BarSource:
    ...
    with pdfspine.open() as document:
        ...
        page.insert_text((10.0, 205.0), ACTUAL_FX_CONTEXT, fontname="Authored", fontsize=3.0)
        if hidden_footer:
            stream = page.get_contents()[-1]
            document.update_stream(stream, document.xref_stream(stream).replace(b"BT", b"BT\n3 Tr"))
        if cover_footer:
        ...
        claims.append(
            {
                ...
                "value": str(point.value.value) if point.value.value is not None else None,
                "period": None,
