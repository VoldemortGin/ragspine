"""W11 隔离 conformance：SPLADE 学习稀疏重排路径下 RESTRICTED 候选绝不被打分/泄漏。

拍板（docs/invariants.md「RESTRICTED isolation 两出口」）：rerank 出口（listwise_rerank）把
sensitivity==RESTRICTED 的候选排除在 judge 之外（不送进打分、原位保留）。SPLADE 作为 ListwiseJudge
接入即【继承】此保护——本文件断言新 reranker 路径下 RESTRICTED 文本绝不进入 SparseTextEmbedding.embed
...
from ragspine.retrieval.rerank.splade import SpladeReranker
...
def _scored_docs(captured) -> list[str]:
    """所有进入 SparseTextEmbedding.embed 的打分文档（跨多次调用展平）。"""
    return [doc for call in captured["embed_calls"] for doc in call]


def test_splade_never_scores_restricted_via_listwise(fake_splade):
    """RESTRICTED 候选经 listwise_rerank 喂给 SPLADE 时：其文本绝不进入 embed 打分文档。"""
    ...
    out = listwise_rerank("a b", results, SpladeReranker())

    scored = _scored_docs(captured)
    assert SECRET not in scored
    assert all(SECRET not in doc for doc in scored)
    # SPLADE 只看到两条非 RESTRICTED 候选。
    assert scored == ["a b public", "a public"]
    # RESTRICTED 块原位保留（未被重排进 judge 输出）。
    assert out[1].chunk.text == SECRET


def test_all_restricted_splade_not_called(fake_splade):
    """候选全 RESTRICTED -> SPLADE 完全不被调用，整体退化为 RRF 序。"""
    captured = fake_splade()
    results = [_result(i, f"secret {i}", sensitivity=RESTRICTED_SENSITIVITY) for i in range(3)]
    out = listwise_rerank("q", results, SpladeReranker())
    assert captured["embed_calls"] == []
    assert captured["query_calls"] == []
    assert [r.chunk.text for r in out] == ["secret 0", "secret 1", "secret 2"]


def test_restricted_case_insensitive_via_splade(fake_splade):
    """sensitivity 大小写不敏感：'restricted' 同样不进 SPLADE 打分。"""
    captured = fake_splade()
    results = [_result(0, SECRET, sensitivity="restricted")]
    listwise_rerank("q", results, SpladeReranker())
    assert captured["embed_calls"] == []


def test_reverse_proof_direct_judge_would_score_restricted(fake_splade):
    """reverse-proof：上面的断言不是空断言。

    若 RESTRICTED 文本【绕过】listwise_rerank 两出口缝、被直接喂给 reranker.judge，SPLADE
    【确实】会把它 embed 打分——这证明隔离来自编排缝（listwise_rerank），SPLADE 接入即继承；
    上面的 conformance 断言能抓住任何绕过该缝的回归（若 listwise_rerank 漏过 RESTRICTED，scored
    里就会出现 SECRET，断言即红）。
    """
    captured = fake_splade()
    SpladeReranker().judge("a b", ["a b public", SECRET, "a public"])
