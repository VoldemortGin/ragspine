...
def _text(cid, doc, page=None, text="正文"):
    h = {"chunk_id": cid, "doc_id": doc, "text": text,
         "source_locator": f"{doc}#page{page}" if page else f"{doc}#para1"}
    return h


def _visual(doc, page, score=0.9):
    return {"doc_id": doc, "page": page, "source_locator": f"{doc}#page{page}",
            "is_visual": True, "scores": {"colpali_maxsim": score}}
...
def test_deterministic():
    ...
    r1 = [h.get("chunk_id") or h.get("source_locator") for h in FusedRetriever(text, visual).retrieve("q")]
    r2 = [h.get("chunk_id") or h.get("source_locator") for h in FusedRetriever(text, visual).retrieve("q")]
    assert r1 == r2
