...
def test_byte_identity_golden(corpus):
    ...
    # 注：c1#0 / c3#0 在 2 个 query 变体下 cosine 完全相等（真实平分）；VectorStore.query
    # 以 (-score, id) 升序破平分 —— 比改造前内联实现更有原则的确定性规则。这使 c3/c1 在那些
    # 平分 ranking 里的相对次序与旧 golden 互换，经 RRF 放大为 ±5.12e-4 的 fused 漂移（仅这两者，
    # bm25/vector 单分及总排序均不变）。良性差异，故按新平分口径刷新这两个 fused_score。
    golden = [
        ("c4#0", 12.236496213582912, 0.7745966692414833, 0.1631411951348493),
        ("c3#0", 4.770858364820141, 0.5619514869490166, 0.16131550452015814),
        ("c1#0", 2.3854291824100704, 0.5677749739576691, 0.15949820788530467),
