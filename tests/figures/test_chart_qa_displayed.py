...
def displayed_context(*, first_display: str = "8.2%") -> DisplayedLookupContext:
    native = b'<svg xmlns="http://www.w3.org/2000/svg" width="240" height="160" viewBox="0 0 240 160"/>'
    ...
    sidecar = TextSidecar(
        ...
        19,
        tuple(
            TextSpan(f"source-{i}", text, bbox) for i, (text, bbox) in enumerate(labels)
        ),
    )
    page = PageInput(
        "d" * 64,
    ...
    if first_display != "8.2%":
        ...
        for element in svg.elements:
            if element.source_span_id == "source-1":
                ...
                node = next(
                    node for node in root.iter() if node.get("id") == element.element_id
                )
    ...
    for category, number in (("1H24", "8.2"), ("1H25", None), ("1H26", "6.9")):
        unit = next(
            ...
            if item.text == "%"
            and item.source_span_id == by_text[number or "8.2"].source_span_id
        )
        points.append(
            ChartPoint(
                ...
                TextField(
                    "%", Evidence((unit.element_id,), Verification.PENDING, confidence)
                ),
                NumericObservation(
    ...
    approved = tuple(
        replace(
            ...
            category=replace(
                point.category, evidence=verified(point.category.evidence)
            ),
    ...
    footnote = "Expense ratio comparatives and two-year changes are shown on an actual exchange rate basis"
    context = PageContextCitation(
        ...
        Verification.VERIFIED,
        Confidence(
            None, "source-qualified page context occurrence; no financial inference"
        ),
    )
...
def test_display_lookup_cites_raw_category_as_period_without_global_period() -> None:
    ...
    assert not any(
        item.element_id == "source-footnote" for item in context.svg.elements
    )
...
def test_scope_or_verified_flags_cannot_replace_exact_raw_and_receipt_bindings() -> (
    None
):
    context = displayed_context()
    ...
    with pytest.raises(ChartQueryError) as caught:
        DisplayedChartQAService(DisplayedResolver(changed)).answer(
            displayed_question(context)
        )
...
def test_incomplete_or_misbound_qualification_never_returns_a_number(
    ...
    match fault:
        ...
        case "raw_period":
            context = replace(
                context,
                raw_chart=replace(
                    context.raw_chart, period=context.raw_chart.points[0].category
                ),
            )
        ...
        case "period_role_confidence":
            context = replace(
                ...
                point_periods=(
                    replace(
                        role, confidence=Confidence(Decimal(1), "invented certainty")
                    ),
                    context.point_periods[1],
                ),
            )
        ...
        case "page_context_source":
            context = replace(
                context,
                page_context=(
                    replace(footnote, source=replace(footnote.source, page_index=18)),
                ),
            )
        case "page_context_crop":
            context = replace(
                context, page_context=(replace(footnote, source=context.svg.source),)
            )
        ...
        case "pending_value":
            ...
            point = replace(
                ...
                value=replace(
                    point.value,
                    evidence=replace(
                        point.value.evidence, verification=Verification.PENDING
                    ),
                ),
            )
            context = replace(
    ...
    with pytest.raises(ChartQueryError) as caught:
        DisplayedChartQAService(DisplayedResolver(context)).answer(
            displayed_question(context)
        )
    assert caught.value.code is QueryFailure.INVALID_EVIDENCE
...
def test_cross_snapshot_port_result_is_a_pin_conflict() -> None:
    context = displayed_context()
    query = replace(
        displayed_question(context), pin=replace(context.pin, snapshot_id="f" * 64)
    )
    with pytest.raises(ChartQueryError) as caught:
...
def test_old_label_only_receipt_cannot_authorize_display_lookup() -> None:
    ...
    context = replace(
        context,
        qualification=replace(
            context.qualification, semantic_scope="figure-source-labels-only-v1"
        ),
    )
    result = DisplayedChartQAService(DisplayedResolver(context)).answer(
        displayed_question(context)
    )
...
@pytest.mark.parametrize(
    "kind", (ValueKind.ESTIMATED, ValueKind.DERIVED, ValueKind.UNAVAILABLE)
)
...
def test_raw_explicit_point_without_qualified_projection_is_a_refusal() -> None:
    ...
    context = replace(
        ...
        description=replace(
            context.description, claims=(context.description.claims[1],)
        ),
        qualification=replace(
...
def test_normalization_audit_must_name_real_typed_branch_identities(fault: str) -> None:
    ...
    if fault == "rule":
        ...
        provenance = replace(
            provenance, normalized_description_artifact_id="unavailable"
        )
    else:
        provenance = replace(
            provenance, binding=replace(provenance.binding, svg_digest="f" * 64)
        )
